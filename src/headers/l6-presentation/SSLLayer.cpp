...
namespace pcpp {
  ...
  SSLLayer *SSLLayer::createSSLMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
    ssl_tls_record_layer *recordLayer = (ssl_tls_record_layer *) data;
  ...
  std::string SSLHandshakeLayer::toString() const {
    ...
    result << getRecordVersion().toString(true) << " AbstractLayer, Handshake:";
  ...
  SSLHandshakeLayer::SSLHandshakeLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
    ...
    while (true) {
      SSLHandshakeMessage *message = SSLHandshakeMessage::createHandshakeMessage(curPos, recordDataLen - curPosIndex,
                                                                                 this);
      if (message == nullptr)
        break;
  ...
  std::string SSLChangeCipherSpecLayer::toString() const {
    std::stringstream result;
    result << getRecordVersion().toString(true) << " AbstractLayer, Change Cipher Spec";
    return result.str();
  }
  ...
  std::string SSLAlertLayer::toString() const {
    std::stringstream result;
    result << getRecordVersion().toString(true) << " AbstractLayer, ";
    if (getAlertLevel() == SSL_ALERT_LEVEL_ENCRYPTED)
  ...
  std::string SSLApplicationDataLayer::toString() const {
    return getRecordVersion().toString(true) + " AbstractLayer, Application Data";
  }
