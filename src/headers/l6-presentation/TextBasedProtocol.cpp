...
namespace pcpp {
  ...
  TextBasedProtocolMessage::TextBasedProtocolMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
      : AbstractLayer(data, dataLen, prevLayer, packet),
        m_FieldList(nullptr), m_LastField(nullptr), m_FieldsOffset(0) {}

  TextBasedProtocolMessage::TextBasedProtocolMessage(const TextBasedProtocolMessage &other) : AbstractLayer(other) {
    copyDataFrom(other);
  }
  ...
  void TextBasedProtocolMessage::parseFields() {
    ...
    HeaderField *firstField = new HeaderField(this, m_FieldsOffset, nameValueSeparator,
                                              spacesAllowedBetweenNameAndValue);
    PCPP_LOG_DEBUG("Added new field: name='" << firstField->getFieldName() << "'; offset in packet="
                                             << firstField->m_NameOffsetInMessage << "; length="
                                             << firstField->getFieldSize());
    ...
    while (!curField->isEndOfHeader() && curOffset + curField->getFieldSize() < m_DataLen) {
      ...
      if (newField->getFieldSize() > 0) {
        PCPP_LOG_DEBUG("Added new field: name='" << newField->getFieldName() << "'; offset in packet="
                                                 << newField->m_NameOffsetInMessage << "; length="
  ...
  HeaderField *TextBasedProtocolMessage::insertField(HeaderField *prevField, const HeaderField &newField) {
    ...
    if (prevField != nullptr && prevField->getFieldName() == PCPP_END_OF_TEXT_BASED_PROTOCOL_HEADER) {
      PCPP_LOG_ERROR("Cannot add a field after end of common");
      return nullptr;
    }
    ...
    if (!extendLayer(newFieldOffset, newFieldToAdd->getFieldSize())) {
      PCPP_LOG_ERROR("Cannot extend layer to insert the common");
  ...
  HeaderField::HeaderField(TextBasedProtocolMessage *TextBasedProtocolMessage, int offsetInMessage,
    ...
    if (fieldEndPtr == nullptr)
      m_FieldSize = tbp_my_own_strnlen(fieldData,
                                       m_TextBasedProtocolMessage->m_DataLen - (size_t) m_NameOffsetInMessage);
    else
    ...
    if (fieldValuePtr == nullptr || (fieldEndPtr != nullptr && fieldValuePtr >= fieldEndPtr)) {
      ...
      if (spacesAllowedBetweenNameAndValue) {
        ...
        while ((size_t) (fieldValuePtr - (char *) m_TextBasedProtocolMessage->m_Data) <
               m_TextBasedProtocolMessage->getDataLen() && (*fieldValuePtr) == ' ') {
          fieldValuePtr++;
        }
      }
      ...
      if ((size_t) (fieldValuePtr - (char *) (m_TextBasedProtocolMessage->m_Data)) >=
        ...
        if (fieldEndPtr == nullptr)
          m_FieldValueSize =
              (char *) (m_TextBasedProtocolMessage->m_Data + m_TextBasedProtocolMessage->getDataLen()) - fieldValuePtr;
        else {
  ...
  HeaderField::HeaderField(const HeaderField &other) : m_NameValueSeparator('\0'),
                                                       m_SpacesAllowedBetweenNameAndValue(false) {
