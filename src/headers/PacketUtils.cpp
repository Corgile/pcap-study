...
namespace pcpp {
  ...
  uint16_t computePseudoHdrChecksum(uint8_t *dataPtr, size_t dataLen, IPAddress::AddressType ipAddrType,
    ...
    PCPP_LOG_DEBUG("Compute pseudo common checksum.\n DataLen = " << dataLen << "IPAddrType = "
                                                                  << ipAddrType << "ProtocolType = " << protocolType
                                                                  << "SrcIP = " << srcIPAddress
                                                                  << "DstIP = " << dstIPAddress);
    ...
    if (ipAddrType == IPAddress::IPv4AddressType) {
      ...
      PCPP_LOG_ERROR("Compute pseudo common checksum failed, for unknown IPAddrType = " << ipAddrType);
    }

    PCPP_LOG_DEBUG("Pseudo common checksum = 0xX" << std::uppercase << std::hex << checksumRes);
