...
namespace pcpp {
  ...
  EthLayer::EthLayer(const MacAddress &sourceMac, const MacAddress &destMac, uint16_t etherType) : AbstractLayer() {
    ...
    switch (be16toh(hdr->etherType)) {
      ...
      case PCPP_ETHERTYPE_WAKE_ON_LAN:
        m_NextLayer = WakeOnLanLayer::isDataValid(payload, payloadLen)
                      ? static_cast<AbstractLayer *>(new WakeOnLanLayer(payload, payloadLen, this, m_Packet))
                      : static_cast<AbstractLayer *>(new PayloadLayer(payload, payloadLen, this, m_Packet));
        break;
      default:
