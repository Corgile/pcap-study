...
namespace pcpp {
  ...
  uint16_t PPPoESessionLayer::getPPPNextProtocol() const {
    if (m_DataLen < getHeaderLen()) {
      PCPP_LOG_ERROR("ERROR: size of layer is smaller then PPPoE session common");
      return 0;
    }
  ...
  std::map<uint16_t, std::string> createPPPNextProtoToStringMap() {
    ...
    tempMap[PCPP_PPP_OSI] = "OSI Network AbstractLayer";
    ...
    tempMap[PCPP_PPP_OSINLCP] = "OSI Network AbstractLayer Control Protocol";
  ...
  PPPoEDiscoveryLayer::addTagAfter(const PPPoETagBuilder &tagBuilder, PPPoETagTypes prevTagType) {
    int offset = 0;
