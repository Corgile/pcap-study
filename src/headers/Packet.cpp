...
#include "Packet.h"
#include "osi/l2-datalink/EthLayer.h"
#include "osi/l2-datalink/EthDot3Layer.h"
#include "osi/l2-datalink/SllLayer.h"
#include "osi/l3-network/NflogLayer.h"
#include "osi/l2-datalink/NullLoopbackLayer.h"
#include "osi/l3-network/IPv4Layer.h"
#include "osi/l3-network/IPv6Layer.h"
#include "osi/l7-application/PayloadLayer.h"
#include "osi/l2-datalink/PacketTrailerLayer.h"
#include "Logger.h"
...
namespace pcpp {
  ...
  void Packet::setRawPacket(RawPacket *rawPacket, bool freeRawPacket, ProtocolType parseUntil,
    ...
    AbstractLayer *curLayer = m_FirstLayer;
  ...
  void Packet::destructPacketData() {
    ...
    while (curLayer != nullptr) {
      AbstractLayer *nextLayer = curLayer->getNextLayer();
  ...
  bool Packet::insertLayer(AbstractLayer *prevLayer, AbstractLayer *newLayer, bool ownInPacket) {
    if (newLayer == nullptr) {
      PCPP_LOG_ERROR("AbstractLayer to add is NULL");
      return false;
    }

    if (newLayer->isAllocatedToPacket()) {
      PCPP_LOG_ERROR("AbstractLayer is already allocated to another packet. Cannot use layer in more than one packet");
      return false;
    }
    ...
    if (m_RawPacket->getRawDataLen() + newLayerHeaderLen > m_MaxPacketLen) {
      if (!m_CanReallocateData) {
        PCPP_LOG_ERROR(
            "With the new layer the packet will exceed the size of the pre-allocated buffer: " << m_MaxPacketLen
                                                                                               << " bytes");
        return false;
      }
  ...
  bool Packet::removeLayer(ProtocolType layerType, int index) {
    AbstractLayer *layerToRemove = getLayerOfType(layerType, index);

    if (layerToRemove != nullptr) {
      return removeLayer(layerToRemove, true);
    } else {
      PCPP_LOG_ERROR("AbstractLayer of the requested type was not found in packet");
      return false;
    }
  }

  bool Packet::removeFirstLayer() {
    AbstractLayer *firstLayer = getFirstLayer();
  ...
  bool Packet::removeAllLayersAfter(AbstractLayer *layer) {
    AbstractLayer *curLayer = layer->getNextLayer();
    while (curLayer != nullptr) {
      AbstractLayer *tempLayer = curLayer->getNextLayer();
  ...
  AbstractLayer *Packet::detachLayer(ProtocolType layerType, int index) {
    AbstractLayer *layerToDetach = getLayerOfType(layerType, index);
  ...
  bool Packet::removeLayer(AbstractLayer *layer, bool tryToDelete) {
    if (layer == nullptr) {
      PCPP_LOG_ERROR("AbstractLayer is NULL");
      return false;
    }

    // verify layer is allocated to a packet
    if (!layer->isAllocatedToPacket()) {
      PCPP_LOG_ERROR("AbstractLayer isn't allocated to any packet");
      return false;
    }

    // verify layer is allocated to *this* packet
    AbstractLayer *curLayer = layer;
    while (curLayer->m_PrevLayer != nullptr)
      curLayer = curLayer->m_PrevLayer;
    if (curLayer != m_FirstLayer) {
      PCPP_LOG_ERROR("AbstractLayer isn't allocated to this packet");
      return false;
    }
  ...
  AbstractLayer *Packet::getLayerOfType(ProtocolType layerType, int index) const {
    AbstractLayer *curLayer = getFirstLayer();
    int curIndex = 0;
    while (curLayer != nullptr) {
  ...
  bool Packet::extendLayer(AbstractLayer *layer, int offsetInLayer, size_t numOfBytesToExtend) {
    if (layer == nullptr) {
      PCPP_LOG_ERROR("AbstractLayer is NULL");
      return false;
    }
    ...
    while (curLayer != nullptr) {
      ...
      // assuming common length of the layer that requested to be extended hasn't been enlarged yet
  ...
  bool Packet::shortenLayer(AbstractLayer *layer, int offsetInLayer, size_t numOfBytesToShorten) {
    if (layer == nullptr) {
      PCPP_LOG_ERROR("AbstractLayer is NULL");
      return false;
    }
  ...
  void Packet::computeCalculateFields() {
    ...
    AbstractLayer *curLayer = m_LastLayer;
    while (curLayer != nullptr) {
  ...
  AbstractLayer *Packet::createFirstLayer(LinkLayerType linkType) {
    ...
    if (linkType == LINKTYPE_ETHERNET) {
      ...
      if (ipVer == 0x40) {
        return IPv4Layer::isDataValid(rawData, rawDataLen)
               ? static_cast<AbstractLayer *>(new IPv4Layer((uint8_t *) rawData, rawDataLen, nullptr, this))
               : static_cast<AbstractLayer *>(new PayloadLayer((uint8_t *) rawData, rawDataLen, nullptr, this));
      } else if (ipVer == 0x60) {
        return IPv6Layer::isDataValid(rawData, rawDataLen)
               ? static_cast<AbstractLayer *>(new IPv6Layer((uint8_t *) rawData, rawDataLen, nullptr, this))
               : static_cast<AbstractLayer *>(new PayloadLayer((uint8_t *) rawData, rawDataLen, nullptr, this));
      } else {
        return new PayloadLayer((uint8_t *) rawData, rawDataLen, nullptr, this);
      }
      ...
      return NflogLayer::isDataValid(rawData, rawDataLen)
             ? static_cast<AbstractLayer *>(new NflogLayer((uint8_t *) rawData, rawDataLen, this))
             : static_cast<AbstractLayer *>(new PayloadLayer((uint8_t *) rawData, rawDataLen, nullptr, this));
    }
