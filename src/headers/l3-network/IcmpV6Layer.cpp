...
#include "osi/l3-network/NdpLayer.h"
...
namespace pcpp {
  ...
  AbstractLayer *IcmpV6Layer::parseIcmpV6Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
  ...
  void IcmpV6Layer::calculateChecksum() {
    /* Pseudo common of 40 bytes which is composed as follows(in order):
    - 16 bytes for the source address
    - 16 bytes for the destination address
    - 4 bytes big endian payload length(the same value as in the IPv6 common)
