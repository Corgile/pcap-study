...
#include "osi/l4-transport/UdpLayer.h"
#include "osi/l4-transport/TcpLayer.h"
#include "osi/l3-network/IcmpLayer.h"
#include "osi/l3-network/GreLayer.h"
#include "osi/l3-network/IgmpLayer.h"
#include "osi/l4-transport/IPSecLayer.h"
#include "osi/l3-network/VrrpLayer.h"
...
#define IPV4_MAX_OPT_SIZE 40
  ...
  IPv4Layer::IPv4Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, bool setTotalLenAsDataLen)
      : AbstractLayer(data, dataLen, prevLayer, packet) {
    initLayerInPacket(setTotalLenAsDataLen);
  }

  IPv4Layer::IPv4Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen,
                                                                                                                prevLayer, packet) {
  ...
  IPv4Layer::IPv4Layer(const IPv4Layer &other) : AbstractLayer(other) {
    copyLayerData(other);
  }

  IPv4Layer &IPv4Layer::operator=(const IPv4Layer &other) {
    AbstractLayer::operator=(other);
  ...
  void IPv4Layer::parseNextLayer() {
    ...
    // TODO: assuming first fragment contains at least L4 common, what if it's not true?
    if (isFragment()) {
    ...
    switch (ipHdr->protocol) {
      ...
      case PACKETPP_IPPROTO_TCP:
        m_NextLayer = TcpLayer::isDataValid(payload, payloadLen)
                      ? static_cast<AbstractLayer *>(new TcpLayer(payload, payloadLen, this, m_Packet))
      ...
      case PACKETPP_IPPROTO_ICMP:
        m_NextLayer = IcmpLayer::isDataValid(payload, payloadLen)
                      ? static_cast<AbstractLayer *>(new IcmpLayer(payload, payloadLen, this, m_Packet))
                      : static_cast<AbstractLayer *>(new PayloadLayer(payload, payloadLen, this, m_Packet));
        break;
      ...
      case PACKETPP_IPPROTO_AH:
        m_NextLayer = AuthenticationHeaderLayer::isDataValid(payload, payloadLen)
                      ? static_cast<AbstractLayer *>(new AuthenticationHeaderLayer(payload, payloadLen, this, m_Packet))
                      : static_cast<AbstractLayer *>(new PayloadLayer(payload, payloadLen, this, m_Packet));
        break;
      case PACKETPP_IPPROTO_ESP:
        m_NextLayer = ESPLayer::isDataValid(payload, payloadLen)
                      ? static_cast<AbstractLayer *>(new ESPLayer(payload, payloadLen, this, m_Packet))
                      : static_cast<AbstractLayer *>(new PayloadLayer(payload, payloadLen, this, m_Packet));
        break;
  ...
  std::string IPv4Layer::toString() const {
    ...
    return "IPv4 AbstractLayer, " + fragment + "Src: " + getSrcIPv4Address().toString() + ", Dst: " +
           getDstIPv4Address().toString();
  }
  ...
  IPv4Option IPv4Layer::addOptionAt(const IPv4OptionBuilder &optionBuilder, int offset) {
    ...
    if (totalOptSize > IPV4_MAX_OPT_SIZE) {
      PCPP_LOG_ERROR(
          "Cannot add option - adding this option will exceed IPv4 total option size which is " << IPV4_MAX_OPT_SIZE);
    ...
    // current size of the common. m_TempHeaderExtension will be added to the length extracted from getIPv4Header()->internetHeaderLength as the temp new size
  ...
  bool IPv4Layer::removeOption(IPv4OptionTypes option) {
    ...
    if (!shortenLayer(offset, sizeToShorten)) {
      PCPP_LOG_ERROR("Failed to remove sf_IPv4 option: cannot shorten layer");
      return false;
    }
