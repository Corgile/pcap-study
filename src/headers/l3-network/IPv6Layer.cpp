...
#include "osi/l3-network/IcmpV6Layer.h"
...
namespace pcpp {
  ...
  IPv6Layer::IPv6Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen,
                                                                                                                prevLayer, packet) {
    m_Protocol = IPv6;
    m_FirstExtension = nullptr;
    m_LastExtension = nullptr;
  ...
  IPv6Layer::IPv6Layer(const IPv6Layer &other) : AbstractLayer(other) {
    m_FirstExtension = nullptr;
    m_LastExtension = nullptr;
    m_ExtensionsLen = 0;
    parseExtensions();
  ...
  std::string IPv6Layer::toString() const {
    std::string result =
        "IPv6 AbstractLayer, Src: " + getSrcIPv6Address().toString() + ", Dst: " + getDstIPv6Address().toString();
