...
#define VRRP_V3_VERSION     3
  ...
  std::string VrrpLayer::toString() const {
    return "VRRP v" + std::to_string(getVersion()) + " AbstractLayer, virtual router ID: " +
           std::to_string(getVirtualRouterID()) + ", IP address count: " + std::to_string(getIPAddressesCount());
  }
  ...
  VrrpV2Layer::VrrpV2Layer(uint8_t virtualRouterId, uint8_t priority, uint8_t advInt, uint8_t authType) : VrrpLayer(
      VRRPv2, virtualRouterId, priority) {
    setAdvInt(advInt);
