...
#include "osi/l3-network/IcmpLayer.h"
#include "osi/l7-application/PayloadLayer.h"
#include "Packet.h"
#include "osi/PacketUtils.h"
#include "Logger.h"
#include <sstream>
#include <string.h>
...
namespace pcpp {
  ...
  IcmpLayer::IcmpLayer() : AbstractLayer() {
  ...
  bool IcmpLayer::setIpAndL4Layers(IPv4Layer *ipLayer, AbstractLayer *l4Layer) {
    if (m_Packet == nullptr) {
  ...
  icmp_echo_request *
  IcmpLayer::setEchoRequestData(uint16_t id, uint16_t sequence, uint64_t timestamp, const uint8_t *data,
                                size_t dataLen) {
  ...
  icmp_echo_reply *
  IcmpLayer::setEchoReplyData(uint16_t id, uint16_t sequence, uint64_t timestamp, const uint8_t *data, size_t dataLen) {
  ...
  IcmpLayer::setTimestampRequestData(uint16_t id, uint16_t sequence, timeval originateTimestamp) {
    if (!cleanIcmpLayer())
      return nullptr;
  ...
  IcmpLayer::setDestUnreachableData(IcmpDestUnreachableCodes code, uint16_t nextHopMTU, IPv4Layer *ipHeader,
                                    AbstractLayer *l4Header) {
    if (!cleanIcmpLayer())
      return nullptr;
  ...
  icmp_source_quench *IcmpLayer::setSourceQuenchdata(IPv4Layer *ipHeader, AbstractLayer *l4Header) {
    if (!cleanIcmpLayer())
      return nullptr;
  ...
  icmp_redirect *
  IcmpLayer::setRedirectData(uint8_t code, IPv4Address gatewayAddress, IPv4Layer *ipHeader, AbstractLayer *l4Header) {
    if (code > 3) {
  ...
  icmp_router_advertisement *IcmpLayer::setRouterAdvertisementData(uint8_t code, uint16_t lifetimeInSeconds,
                                                                   const std::vector<icmp_router_address_structure> &routerAddresses) {
    if (code != 0 && code != 16) {
      PCPP_LOG_ERROR(
          "Unknown code " << (int) code << " for ICMP router advertisement data (only codes 0 and 16 are legal)");
      return nullptr;
    }
    ...
    icmp_router_address_structure *curPos = (icmp_router_address_structure *) ((uint8_t *) header->header +
                                                                               sizeof(icmp_router_advertisement_hdr));
    for (std::vector<icmp_router_address_structure>::const_iterator iter = routerAddresses.begin();
  ...
  icmp_time_exceeded *IcmpLayer::setTimeExceededData(uint8_t code, IPv4Layer *ipHeader, AbstractLayer *l4Header) {
    if (code > 1) {
  ...
  icmp_param_problem *
  IcmpLayer::setParamProblemData(uint8_t code, uint8_t errorOctetPointer, IPv4Layer *ipHeader, AbstractLayer *l4Header) {
    if (code > 2) {
    ...
    switch (getMessageType()) {
      ...
      case ICMP_PARAM_PROBLEM:
        m_NextLayer = IPv4Layer::isDataValid(m_Data + headerLen, m_DataLen - headerLen)
                      ? static_cast<AbstractLayer *>(new IPv4Layer(m_Data + headerLen, m_DataLen - headerLen, this, m_Packet))
                      : static_cast<AbstractLayer *>(new PayloadLayer(m_Data + headerLen, m_DataLen - headerLen, this,
                                                                      m_Packet));
        return;
      default:
  ...
  std::string IcmpLayer::toString() const {
    ...
    switch (type) {
      ...
      case ICMP_TIME_EXCEEDED:
        ...
        break;
      case ICMP_PARAM_PROBLEM:
        messageTypeAsString = "Parameter problem: bad IP common";
        break;
      case ICMP_TIMESTAMP_REQUEST:
    ...
    return "ICMP AbstractLayer, " + messageTypeAsString + " (type: " + typeStream.str() + ")";
  }
