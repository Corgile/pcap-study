...
#include "osi/l3-network/IPReassembly.h"
...
namespace pcpp {
  ...
  public:
    ...
    IPReassembly::PacketKey *createPacketKey() override {
      return new IPReassembly::IPv4PacketKey(be16toh(m_IPLayer->getIPv4Header()->ipId), m_IPLayer->getSrcIPv4Address(),
                                             m_IPLayer->getDstIPv4Address());
    }
  ...
  public:
    ...
    IPReassembly::PacketKey *createPacketKey() override {
      return new IPReassembly::IPv6PacketKey(be32toh(m_FragHeader->getFragHeader()->id), m_IPLayer->getSrcIPv6Address(),
                                             m_IPLayer->getDstIPv6Address());
    }
  ...
  Packet *IPReassembly::processPacket(Packet *fragment, ReassemblyStatus &status, ProtocolType parseUntil,
    ...
    if (iter == m_FragmentMap.end()) {
      PCPP_LOG_DEBUG(
          "Got new packet with FragID=0x" << std::hex << fragWrapper->getFragmentId() << ", allocating place in map");
    ...
    if (fragWrapper->isFirstFragment()) {
      ...
      {
        PCPP_LOG_DEBUG(
            "[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got first fragment, allocating RawPacket");
        ...
        auto rawDataLen =
            fragWrapper->getIPLayerPayload() - fragmentRawPacket->getRawData() + fragWrapper->getIPLayerPayloadSize();
    ...
    } else // not first fragment
    {
      ...
      if (fragData->currentOffset == fragOffset) {
        ...
        PCPP_LOG_DEBUG(
            "[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Found next matching fragment with offset "
                         << fragOffset << ", adding fragment data to reassembled packet");
        ...
        PCPP_LOG_DEBUG(
            "[FragID=0x" << std::hex << fragWrapper->getFragmentId() << "] Got out-of-ordered fragment with offset "
  ...
  void IPReassembly::addNewFragment(uint32_t hash, IPFragmentData *fragData) {
        ...
        1) // this means LRU list was full and the least recently used item was removed
    {
      ...
      PCPP_LOG_DEBUG(
          "Reached maximum packet capacity, removing data for FragID=0x" << std::hex << dataRemoved->fragmentID);
      delete dataRemoved;
      m_FragmentMap.erase(iter);
    ...
    while (!foundLastSegment) {
      ...
      while (index < (int) fragData->outOfOrderFragments.size()) {
        ...
        if (fragData->currentOffset == frag->fragmentOffset) {
          ...
          {
            PCPP_LOG_DEBUG(
                "[FragID=0x" << std::hex << fragData->fragmentID << "] Found last fragment inside out-of-order list");
      ...
      if (!foundOutOfOrderFrag) {
        ...
        PCPP_LOG_DEBUG(
            "[FragID=0x" << std::hex << fragData->fragmentID << "] Didn't find the next fragment in out-of-order list");
        break;
      }
    }
