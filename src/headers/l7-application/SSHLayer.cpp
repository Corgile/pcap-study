...
#define SSH_LAYER_BASE_STRING "SSH AbstractLayer"
  ...
  SSHLayer *SSHLayer::createSSHMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
  ...
  SSHIdentificationMessage::tryParse(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
  ...
  SSHHandshakeMessage *SSHHandshakeMessage::tryParse(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
    ...
    if (msgBase->messageCode != 20 &&
      ...
      PCPP_LOG_DEBUG(
          "Unknown message type " << (int) msgBase->messageCode << ". It's probably not an SSH handshake message");
      return nullptr;
    }
  ...
  SSHKeyExchangeInitMessage::SSHKeyExchangeInitMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
