...
#include "osi/l7-application/SomeIpSdLayer.h"
#include "Packet.h"
...
namespace pcpp {
  ...
  AbstractLayer *SomeIpLayer::parseSomeIpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
    ...
    uint32_t lengthBE = 0;
    memcpy(&lengthBE, data + sizeof(uint32_t), sizeof(uint32_t)); // length field in SOME/IP common
  ...
  std::string SomeIpLayer::toString() const {
    ...
    dataStream << "SOME/IP AbstractLayer"
               << std::hex
  ...
  std::string SomeIpTpLayer::toString() const {
    ...
    dataStream << "SOME/IP-TP AbstractLayer"
               << std::hex
               << ", Service ID: 0x" << getServiceID()
               << ", Method ID: 0x" << getMethodID()
               << std::dec
