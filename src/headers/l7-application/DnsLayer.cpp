...
#include "osi/l7-application/DnsLayer.h"
#include "Logger.h"
#include "IpAddress.h"
#include <sstream>
#include <string.h>
...
namespace pcpp {
  ...
  DnsLayer::DnsLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
  ...
  DnsLayer::DnsLayer(const DnsLayer &other) : AbstractLayer(other) {
    init(other.m_OffsetAdjustment, true);
  }

  DnsLayer::DnsLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, size_t offsetAdjustment)
      : AbstractLayer(data, dataLen, prevLayer, packet) {
  ...
  bool DnsLayer::extendLayer(int offsetInLayer, size_t numOfBytesToExtend, IDnsResource *resource) {
    if (!AbstractLayer::extendLayer(offsetInLayer, numOfBytesToExtend))
      return false;
  ...
  bool DnsLayer::shortenLayer(int offsetInLayer, size_t numOfBytesToShorten, IDnsResource *resource) {
    if (!AbstractLayer::shortenLayer(offsetInLayer, numOfBytesToShorten))
      return false;
  ...
  DnsResource *
  DnsLayer::addResource(DnsResourceType resType, const std::string &name, DnsType dnsType, DnsClass dnsClass,
  ...
  DnsResource *DnsLayer::addAnswer(DnsResource *const copyAnswer) {
    ...
    return addAnswer(copyAnswer->getName(), copyAnswer->getDnsType(), copyAnswer->getDnsClass(), copyAnswer->getTTL(),
  ...
  DnsResource *DnsLayer::addAdditionalRecord(const std::string &name, DnsType dnsType, DnsClass dnsClass, uint32_t ttl,
                                             IDnsResourceData *data) {
  ...
  DnsResource *
  DnsLayer::addAdditionalRecord(const std::string &name, DnsType dnsType, uint16_t customData1, uint32_t customData2,
                                IDnsResourceData *data) {
