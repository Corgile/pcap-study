...
#include <string.h>
#include "Logger.h"
#include "osi/l7-application/BgpLayer.h"
...
namespace pcpp {
  ...
  BgpLayer *BgpLayer::parseBgpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) {
    if (dataLen < sizeof(bgp_common_header))
      return nullptr;

    bgp_common_header *bgpHeader = (bgp_common_header *) data;

    // illegal common data - length is too small
  ...
  size_t BgpOpenMessageLayer::optionalParamsToByteArray(const std::vector<optional_parameter> &optionalParams,
    ...
    for (std::vector<optional_parameter>::const_iterator iter = optionalParams.begin();
  ...
  void
  BgpUpdateMessageLayer::parsePrefixAndIPData(uint8_t *dataPtr, size_t dataLen, std::vector<prefix_and_ip> &result) {
    size_t byteCount = 0;
  ...
  size_t BgpUpdateMessageLayer::prefixAndIPDataToByteArray(const std::vector<prefix_and_ip> &prefixAndIpData,
    ...
    for (std::vector<prefix_and_ip>::const_iterator iter = prefixAndIpData.begin();
  ...
  size_t BgpUpdateMessageLayer::pathAttributesToByteArray(const std::vector<path_attribute> &pathAttributes,
    ...
    for (std::vector<path_attribute>::const_iterator iter = pathAttributes.begin();
  ...
  bool BgpUpdateMessageLayer::setWithdrawnRoutes(const std::vector<prefix_and_ip> &withdrawnRoutes) {
    ...
    getBasicHeader()->length = htobe16(
        be16toh(getBasicHeader()->length) + newWithdrawnRoutesDataLen - curWithdrawnRoutesDataLen);
  ...
  bool BgpUpdateMessageLayer::setPathAttributes(const std::vector<path_attribute> &pathAttributes) {
    ...
    getBasicHeader()->length = htobe16(
        be16toh(getBasicHeader()->length) + newPathAttributesDataLen - curPathAttributesDataLen);
  ...
  bool BgpUpdateMessageLayer::setNetworkLayerReachabilityInfo(const std::vector<prefix_and_ip> &nlri) {
    ...
    if (newNlriDataLen > 0) {
      memcpy(m_Data + sizeof(bgp_common_header) + 2 * sizeof(uint16_t) + curWithdrawnRoutesDataLen +
             curPathAttributesDataLen, newNlriData, newNlriDataLen);
    }
  ...
  void
  BgpNotificationMessageLayer::initMessageData(uint8_t errorCode, uint8_t errorSubCode, const uint8_t *notificationData,
                                               size_t notificationDataLen) {
  ...
  bool
  BgpNotificationMessageLayer::setNotificationData(const uint8_t *newNotificationData, size_t newNotificationDataLen) {
