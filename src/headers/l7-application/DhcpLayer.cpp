...
#define DHCP_MAGIC_NUMBER 0x63538263
  ...
  DhcpOption DhcpOptionBuilder::build() const {
    ...
    if ((recType == DHCPOPT_END || recType == DHCPOPT_PAD)) {
      if (m_RecValueLen != 0) {
        PCPP_LOG_ERROR("Can't set DHCP END option or DHCP PAD option with size different than 0, tried to set size "
                           << (int) m_RecValueLen);
  ...
  DhcpLayer::DhcpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen,
                                                                                                                prevLayer, packet) {
    m_Protocol = DHCP;
  }
  ...
  DhcpLayer::DhcpLayer() : AbstractLayer() {
    initDhcpLayer(sizeof(dhcp_header));
  }

  DhcpLayer::DhcpLayer(DhcpMessageType msgType, const MacAddress &clientMacAddr) : AbstractLayer() {
