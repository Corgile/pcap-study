...
namespace pcpp {
  ...
  HeaderField *
  HttpMessage::insertField(HeaderField *prevField, const std::string &fieldName, const std::string &fieldValue) {
    if (getFieldByName(fieldName) != nullptr) {
      PCPP_LOG_ERROR("Field '" << fieldName << "' already exists!");
      return nullptr;
    }
  ...
  HttpRequestLayer::HttpRequestLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : HttpMessage(
      data, dataLen, prevLayer, packet) {
  ...
  HttpResponseLayer::HttpResponseLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : HttpMessage(
      data, dataLen, prevLayer, packet) {
  ...
  HttpResponseFirstLine::HttpResponseFirstLine(HttpResponseLayer *httpResponse) : m_HttpResponse(httpResponse) {
    ...
    if (Logger::getInstance().isDebugEnabled(PacketLogModuleHttpLayer)) {
      ...
      int statusCode = (m_StatusCode == HttpResponseLayer::HttpStatusCodeUnknown ? 0
