...
namespace pcpp {
   ...
   *  SomeIpSdLayer
   */
  SomeIpSdLayer::SomeIpSdLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
  ...
  std::string SomeIpSdLayer::toString() const {
    ...
    dataStream << "SOME/IP-SD AbstractLayer, " << getNumEntries() << " entries, " << getNumOptions() << " options";
