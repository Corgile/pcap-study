...
#include <sstream>
#include "Logger.h"
#include "osi/l4-transport/GtpLayer.h"
...
#define PCPP_GTP_V1_GPDU_MESSAGE_TYPE 0xff
  ...
  GtpV1Layer::GtpExtension::createGtpExtension(uint8_t *data, size_t dataLen, uint8_t extType, uint16_t content) {
  ...
  bool GtpV1Layer::setSequenceNumber(const uint16_t seqNumber) {
    ...
    gtpv1_header *header = getHeader();
    if (header == nullptr) {
      PCPP_LOG_ERROR("Set sequence failed: GTP common is NULL");
      return false;
    }

    // if all flags are unset then create the GTP extra common
    ...
    // get the extra common
    gtpv1_header_extra *headerExtra = getHeaderExtra();
    if (headerExtra == nullptr) {
      PCPP_LOG_ERROR("Set sequence failed: extra common is NULL");
      return false;
    }
  ...
  bool GtpV1Layer::setNpduNumber(const uint8_t npduNum) {
    ...
    gtpv1_header *header = getHeader();
    if (header == nullptr) {
      PCPP_LOG_ERROR("Set N-PDU failed: GTP common is NULL");
      return false;
    }
    ...
    gtpv1_header_extra *headerExtra = getHeaderExtra();
    if (headerExtra == nullptr) {
      PCPP_LOG_ERROR("Set N-PDU failed: extra common is NULL");
      return false;
    }
  ...
  GtpV1Layer::GtpExtension GtpV1Layer::addExtension(uint8_t extensionType, uint16_t extensionContent) {
    ...
    gtpv1_header *header = getHeader();
    if (header == nullptr) {
      PCPP_LOG_ERROR("Add extension failed: GTP common is NULL");
      return GtpExtension();
    }
    ...
    gtpv1_header_extra *headerExtra = getHeaderExtra();
    if (headerExtra == nullptr) {
      PCPP_LOG_ERROR("Add extension failed: extra common is NULL");
      return GtpExtension();
    }

    offsetForNewExtension += sizeof(gtpv1_header_extra);

    // find the last GTP common extension
    GtpV1Layer::GtpExtension lastExt = getNextExtension();

    // go over the GTP common extensions
    ...
    // lastExt != null means layer contains 1 or more extensions
    if (!lastExt.isNull()) {
      // set the next common type in the last extension
