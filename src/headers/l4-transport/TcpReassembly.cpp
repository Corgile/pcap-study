...
#include "osi/l4-transport/TcpReassembly.h"
#include "osi/l4-transport/TcpLayer.h"
#include "osi/l3-network/IPLayer.h"
...
namespace pcpp {
  ...
  TcpReassembly::ReassemblyStatus TcpReassembly::reassemblePacket(Packet &tcpData) {
    ...
    if (iter == m_ConnectionList.end()) {
      ...
      std::pair<ConnectionList::iterator, bool> pair = m_ConnectionList.insert(
          std::make_pair(flowKey, TcpReassemblyData()));
    ...
    if (tcpReassemblyData->twoSides[sideIndex].gotFinOrRst) {
      PCPP_LOG_DEBUG(
          "Got a packet after FIN or RST were already seen on this side (" << sideIndex << "). Ignoring this packet");
      return Ignore_PacketOfClosedFlow;
    }
    ...
    if (m_EnableBaseBufferClearCondition && !first && tcpPayloadSize > 0 && tcpReassemblyData->prevSide != -1 &&
      ...
      PCPP_LOG_DEBUG(
          "Seeing a first data packet from a different side. Previous side was " << tcpReassemblyData->prevSide
                                                                                 << ", current side is " << sideIndex);
    ...
    if (SEQ_LT(sequence, tcpReassemblyData->twoSides[sideIndex].sequence)) {
      ...
      PCPP_LOG_DEBUG("Found out-of-order packet and added a new TCP fragment with size " << tcpPayloadSize
                                                                                         << " to the out-of-order list of side "
                                                                                         << sideIndex);
  ...
  void
  TcpReassembly::handleFinOrRst(TcpReassemblyData *tcpReassemblyData, int8_t sideIndex, uint32_t flowKey, bool isRst) {
  ...
  void TcpReassembly::checkOutOfOrderFragments(TcpReassemblyData *tcpReassemblyData, int8_t sideIndex,
    ...
    do {
        ...
        while (index < (int) tcpReassemblyData->twoSides[sideIndex].tcpFragmentList.size()) {
          ...
          if (curTcpFrag->sequence == tcpReassemblyData->twoSides[sideIndex].sequence) {
            ...
            if (curTcpFrag->data != nullptr) {
              PCPP_LOG_DEBUG(
                  "Found an out-of-order packet matching to the current sequence with size " << curTcpFrag->dataLength
                                                                                             << " on side " << sideIndex
          ...
          if (SEQ_LT(curTcpFrag->sequence, tcpReassemblyData->twoSides[sideIndex].sequence)) {
            ...
            if (SEQ_GT(newSequence, tcpReassemblyData->twoSides[sideIndex].sequence)) {
              ...
              PCPP_LOG_DEBUG(
                  ...
                  "Calling the callback with the new data. Fragment size is " << curTcpFrag->dataLength << " on side "
                                                                              << sideIndex << ", new data size is "
                                                                              << (int) (curTcpFrag->dataLength -
                                                                                        newLength));
              ...
              // send only the new data to the callback
              if (m_OnMessageReadyCallback != nullptr) {
                TcpStreamData streamData(curTcpFrag->data + newLength, curTcpFrag->dataLength - newLength, 0,
      ...
      if (!cleanWholeFragList && (m_MaxOutOfOrderFragments == 0 ||
      ...
      if (closestSequenceFragIndex > -1) {
        ...
        if (curTcpFrag->data != nullptr) {
          ...
          if (m_OnMessageReadyCallback != nullptr) {
            ...
            TcpStreamData streamData(&dataWithMissingDataText[0], dataWithMissingDataText.size(), missingDataLen,
                                     tcpReassemblyData->connData, curTcpFrag->timestamp);
            m_OnMessageReadyCallback(sideIndex, streamData, m_UserCookie);

            PCPP_LOG_DEBUG("Found missing data on side " << sideIndex << ": " << missingDataLen
  ...
  void TcpReassembly::insertIntoCleanupList(uint32_t flowKey) {
    ...
    std::pair<CleanupList::iterator, bool> pair = m_CleanupList.insert(
        std::make_pair(time(nullptr) + m_ClosedConnectionDelay, CleanupList::mapped_type()));
