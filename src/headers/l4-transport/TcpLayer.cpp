...
#include "osi/l7-application/HttpLayer.h"
#include "osi/l6-presentation/SSLLayer.h"
#include "osi/l5-session/SipLayer.h"
#include "osi/l7-application/BgpLayer.h"
#include "osi/l7-application/SSHLayer.h"
#include "osi/l7-application/DnsLayer.h"
#include "osi/l7-application/TelnetLayer.h"
#include "osi/l4-transport/TpktLayer.h"
#include "osi/l7-application/FtpLayer.h"
...
#define TCPOPT_DUMMY 0xff
  ...
  TcpLayer::TcpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                              packet) {
    m_Protocol = TCP;
    m_NumOfTrailingBytes = 0;
  }
  ...
  TcpLayer::TcpLayer(const TcpLayer &other) : AbstractLayer(other) {
    copyLayerData(other);
  }
  ...
  void TcpLayer::parseNextLayer() {
    ...
    if (HttpMessage::isHttpPort(portDst) &&
      ...
      m_NextLayer = new HttpRequestLayer(payload, payloadLen, this, m_Packet);
             ...
             HttpResponseFirstLine::parseVersion((char *) payload, payloadLen) != HttpVersion::HttpVersionUnknown &&
  ...
  std::string TcpLayer::toString() const {
    ...
    std::string result = "TCP AbstractLayer, ";
    if (hdr->synFlag) {
      if (hdr->ackFlag)
        result += "[SYN, ACK], ";
      else
