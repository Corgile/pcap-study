...
#include "osi/l5-session/SdpLayer.h"
...
namespace pcpp {
  ...
  SipRequestFirstLine::SipRequestFirstLine(SipRequestLayer *sipRequest) : m_SipRequest(sipRequest) {
    ...
    if (Logger::getInstance().isDebugEnabled(PacketLogModuleSipLayer)) {
      std::string method = (m_Method == SipRequestLayer::SipMethodUnknown ? "Unknown"
  ...
  SipRequestLayer::SipRequestLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : SipLayer(data,
                                                                                                                       dataLen,
                                                                                                                       prevLayer,
                                                                                                                       packet) {
  ...
  SipResponseLayer::SipResponseLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : SipLayer(data,
                                                                                                                         dataLen,
                                                                                                                         prevLayer,
                                                                                                                         packet) {
  ...
  SipResponseFirstLine::SipResponseFirstLine(SipResponseLayer *sipResponse) : m_SipResponse(sipResponse) {
    ...
    if (Logger::getInstance().isDebugEnabled(PacketLogModuleSipLayer)) {
      ...
      PCPP_LOG_DEBUG(
          "Version='" << m_Version << "'; Status code=" << statusCode << " '" << getStatusCodeString() << "'");
    }
  }
