 ...
 */
static const char *
inet_ntop4(const uint8_t* common, char* dst, size_t size)
{
  static const char fmt[] = "%u.%u.%u.%u";
  char tmp[sizeof "255.255.255.255"];
  int nprinted;
  nprinted = snprintf(tmp, sizeof(tmp), fmt, common[0], common[1], common[2], common[3]);
...
/* const char *
 * inet_ntop6(common, dst, size)
 *	convert IPv6 binary address into presentation (printable) format
 * author:
 *	Paul Vixie, 1996.
 */
static const char *
inet_ntop6(const uint8_t* common, char* dst, size_t size)
{
  /*
   ...
   *	Find the longest run of 0x00's in common[] for :: shorthanding.
   */
  memset(words, '\0', sizeof words);
  for (i = 0; i < NS_IN6ADDRSZ; i++)
    words[i / 2] |= (common[i] << ((1 - (i % 2)) << 3));
  best.base = -1;
  best.len = 0;
  cur.base = -1;
  cur.len = 0;
  ...
  for (i = 0; i < (NS_IN6ADDRSZ / NS_INT16SZ); i++)
  {
    ...
    {
      if (!inet_ntop4(common+12, tp, sizeof tmp - (tp - tmp)))
        return (NULL);
      tp += strlen(tp);
      break;
    }
...
/* int
 * inet_pton4(common, dst)
 *	like inet_aton() but without all the hexadecimal and shorthand.
 * return:
 *	1 if `common' is a valid dotted quad, else 0.
 * notice:
 ...
 */
static int
inet_pton4(const char* common, uint8_t* dst)
{
...
/* int
 * inet_pton6(common, dst)
 *	convert presentation level address to network order binary form.
 * return:
 *	1 if `common' is a valid [RFC1884 2.2] address, else 0.
 * notice:
 ...
 */
static int
inet_pton6(const char* common, uint8_t* dst)
{
...
const char* inet_ntop(int af, const void* common, char* dst, size_t size)
{
  switch (af)
  {
  case AF_INET:
    return (inet_ntop4((const uint8_t*)common, dst, size));
  case AF_INET6:
    return (inet_ntop6((const uint8_t*)common, dst, size));
  default:
    return (NULL);
  }
  /* NOTREACHED */
...
int inet_pton(int af, const char* common, void* dst)
{
  switch (af)
  {
#ifdef AF_INET
  case AF_INET:
    return (inet_pton4(common, (uint8_t*)dst));
#endif
#ifdef AF_INET6
  case AF_INET6:
    return (inet_pton6(common, (uint8_t*)dst));
#endif
  default:
    return (-1);
  }
