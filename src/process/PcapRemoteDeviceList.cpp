...
namespace pcpp {
  ...
  PcapRemoteDeviceList *PcapRemoteDeviceList::getRemoteDeviceList(const IPAddress &ipAddress, uint16_t port,
    ...
    portAsString << port;
    if (pcap_createsrcstr(remoteCaptureString, PCAP_SRC_IFREMOTE, ipAddress.toString().c_str(),
    ...
    if (remoteAuth != NULL) {
      PCPP_LOG_DEBUG(
          "Authentication requested. Username: " << remoteAuth->userName << ", Password: " << remoteAuth->password);
  ...
  PcapRemoteDevice *PcapRemoteDeviceList::getRemoteDeviceByIP(const IPv4Address &ip4Addr) const {
    ...
    for (ConstRemoteDeviceListIterator devIter = m_RemoteDeviceList.begin();
      ...
      for (std::vector<pcap_addr_t>::iterator addrIter = (*devIter)->m_Addresses.begin();
