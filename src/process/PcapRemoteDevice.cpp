...
namespace pcpp {
  ...
  PcapRemoteDevice::PcapRemoteDevice(pcap_if_t *iface, PcapRemoteAuthentication *remoteAuthentication,
  ...
  void PcapRemoteDevice::getStatistics(PcapStats &stats) const {
    ...
    if (allocatedMemory < (int) sizeof(pcap_stat)) {
      PCPP_LOG_ERROR(
          "Error getting statistics from live device '" << m_Name << "': WinPcap did not allocate the entire struct");
      return;
    }
