...
namespace pcpp {
  ...
  static void arpPacketReceived(RawPacket *rawPacket, PcapLiveDevice *, void *userCookie) {
    ...
    auto *data = (ArpingReceivedData *) userCookie;
    ...
    auto *arpReplyLayer = packet.getLayerOfType<ArpLayer>(true); // lookup in reverse order
  ...
  static void dnsResponseReceived(RawPacket *rawPacket, PcapLiveDevice *, void *userCookie) {
    ...
    auto *data = (DNSReceivedData *) userCookie;
    ...
    auto *dnsResponseLayer = packet.getLayerOfType<DnsLayer>(true); // lookup in reverse order
  ...
  IPv4Address
  NetworkUtils::getIPv4Address(const std::string &hostname, PcapLiveDevice *device, double &dnsResponseTimeMS,
                               uint32_t &dnsTTL,
