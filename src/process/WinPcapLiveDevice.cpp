...
namespace pcpp {
  ...
  WinPcapLiveDevice::WinPcapLiveDevice(pcap_if_t *iface, bool calculateMTU, bool calculateMacAddress,
                                       bool calculateDefaultGateway) : PcapLiveDevice(iface, calculateMTU,
                                                                                      calculateMacAddress,
  ...
  int WinPcapLiveDevice::sendPackets(RawPacket *rawPacketsArr, int arrLength) {
    ...
    if ((res = pcap_sendqueue_transmit(m_PcapDescriptor, sendQueue, 0)) < (int) (sendQueue->len)) {
      PCPP_LOG_ERROR("An error occurred sending the packets: " << pcap_geterr(m_PcapDescriptor) << ". Only " << res
                                                               << " bytes were sent");
      packetsSent = 0;
      dataSize = 0;
      for (int i = 0; i < arrLength; i++) {
