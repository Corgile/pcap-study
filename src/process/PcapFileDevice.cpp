...
namespace pcpp {
  ...
  bool SnoopFileReaderDevice::open() {
    ...
    if (!m_snoopFile) {
      PCPP_LOG_ERROR("Cannot read snoop file common for '" << m_FileName << "'");
      m_snoopFile.close();
      return false;
    }
  ...
  bool PcapFileReaderDevice::getNextPacket(RawPacket &rawPacket) {
    ...
    timespec ts = {pkthdr.ts.tv_sec,
                   static_cast<long>(pkthdr.ts.tv_usec)}; //because we opened with nano second precision 'tv_usec' is actually nanos
#else
  ...
  bool PcapFileWriterDevice::open() {
    ...
    if (m_PcapDumpHandler == nullptr) {
      PCPP_LOG_ERROR(
          "Error opening file writer device for file '" << m_FileName << "': pcap_dump_open returned NULL with error: '"
  ...
  bool PcapFileWriterDevice::open(bool appendMode) {
    ...
    if (amountRead != sizeof(pcap_file_header)) {
      if (ferror(m_File))
        PCPP_LOG_ERROR("Cannot read pcap common from file '" << m_FileName << "', error was: " << errno);
      else
        PCPP_LOG_ERROR("Cannot read pcap common from file '" << m_FileName << "', unknown error");
    ...
    if (linkLayerType != m_PcapLinkLayerType) {
      PCPP_LOG_ERROR("Pcap file has a different link layer type than the one chosen in PcapFileWriterDevice constructor, "
  ...
  PcapNgFileWriterDevice::PcapNgFileWriterDevice(const std::string &fileName, int compressionLevel) : IFileWriterDevice(
      fileName) {
    m_LightPcapNg = nullptr;
  ...
  bool PcapNgFileWriterDevice::open(const std::string &os, const std::string &hardware, const std::string &captureApp,
    ...
    light_pcapng_file_info *info = light_create_file_info(os.c_str(), hardware.c_str(), captureApp.c_str(),
                                                          fileComment.c_str());

    m_LightPcapNg = light_pcapng_open_write(m_FileName.c_str(), info, m_CompressionLevel);
    if (m_LightPcapNg == nullptr) {
      PCPP_LOG_ERROR(
          "Error opening file writer device for file '" << m_FileName << "': light_pcapng_open_write returned NULL");
  ...
  bool PcapNgFileWriterDevice::open(bool appendMode) {
    ...
    if (m_LightPcapNg == nullptr) {
      PCPP_LOG_ERROR("Error opening file writer device in append mode for file '" << m_FileName
