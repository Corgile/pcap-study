...
// This defines statement eliminates these errors
...
namespace pcpp {
  ...
  PcapLiveDevice::PcapLiveDevice(pcap_if_t *pInterface, bool calculateMTU, bool calculateMacAddress,
                                 bool calculateDefaultGateway) : IPcapDevice(),
                                                                 m_MacAddress(""),
  ...
  void PcapLiveDevice::onPacketArrives(uint8_t *user, const struct pcap_pkthdr *pkthdr, const uint8_t *packet) {
    auto *pThis = (PcapLiveDevice *) user;
    if (pThis == nullptr) {
  ...
  void
  PcapLiveDevice::onPacketArrivesNoCallback(uint8_t *user, const struct pcap_pkthdr *pkthdr, const uint8_t *packet) {
    auto *pThis = (PcapLiveDevice *) user;
    if (pThis == nullptr) {
      PCPP_LOG_ERROR("Unable to extract PcapLiveDevice instance");
      return;
    ...
    auto *packetData = new uint8_t[pkthdr->caplen];
    memcpy(packetData, packet, pkthdr->caplen);
    auto *rawPacketPtr = new RawPacket(packetData, pkthdr->caplen, pkthdr->ts, true, pThis->getLinkType());
  ...
  bool PcapLiveDevice::startCapture(OnPacketArrivesCallback onPacketArrives, void *onPacketArrivesUserCookie,
    ...
    PCPP_LOG_DEBUG("Successfully created capture thread for device '" << m_Name << "'. Thread id: "
                                                                      << m_CaptureThread.get_id());
    ...
    if (onStatsUpdate != nullptr && intervalInSecondsToUpdateStats > 0) {
      ...
      PCPP_LOG_DEBUG("Successfully created stats thread for device '" << m_Name << "'. Thread id: "
                                                                      << m_StatsThread.get_id());
    }
  ...
  bool PcapLiveDevice::captureActive() const {
  ...
  bool PcapLiveDevice::doMtuCheck(int packetPayloadLength) const {
    if (packetPayloadLength > (int) m_DeviceMtu) {
      PCPP_LOG_ERROR(
          "Payload length [" << packetPayloadLength << "] is larger than device MTU [" << m_DeviceMtu << "]");
      return false;
    }
    return true;
  }
  ...
  void PcapLiveDevice::setDeviceMtu() {
    ...
    uint8_t buffer[512];
    auto *oidData = (PACKET_OID_DATA *) buffer;
    ...
    if (PacketRequest(adapter, false, oidData)) {
      if (oidData->Length <= sizeof(uint32_t)) {
        ...
        // Sometimes the query gives a wrong number that includes the link common size
...
#pragma GCC diagnostic pop
  ...
  void PcapLiveDevice::setDefaultGateway() {
    ...
    auto *buffer = new uint8_t[outBufLen];
    auto adapterInfo = (IP_ADAPTER_INFO *) buffer;
    DWORD retVal = 0;

    retVal = GetAdaptersInfo(adapterInfo, &outBufLen);
    auto *buffer2 = new uint8_t[outBufLen];
  ...
  const std::vector<IPv4Address> &PcapLiveDevice::getDnsServers() {
    return PcapLiveDeviceList::getInstance().getDnsServers();
  }

  PcapLiveDevice::~PcapLiveDevice() = default;
