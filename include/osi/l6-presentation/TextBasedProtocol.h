...
#define PCPP_END_OF_TEXT_BASED_PROTOCOL_HEADER ""
  ...
  /**
   * @class HeaderField
   * A wrapper class for each text-based-protocol common field, e.g "Host", "Cookie", "Content-Length", "Via", "Call-ID", etc.
   ...
   * The separator between common fields is either CRLF ("\r\n\") or LF ("\n") in more rare cases, which means every HeaderField instance is
   * responsible for wrapping and parsing a common field from the previous CRLF (not inclusive) until the next CRLF/LF (inclusive)
   * A special case is with the end of a common, meaning 2 consecutive CRLFs ("\r\n\r\n") or consecutive LFs ("\n\n"). PcapPlusPlus treats the first
   * CRLF/LF as part of the last field in the common, and the second CRLF is an HeaderField instance of its own which name and values are an empty string ("")
  ...
  public:
    ...
    /**
     * Get an indication whether the field is a field that ends the common (meaning contain only CRLF - see class explanation)
     * @return True if this is a end-of-common field, false otherwise
     */
  ...
  /**
   * @class TextBasedProtocolMessage
   * An abstract base class that wraps text-based-protocol common layers (both requests and responses). It is the base class for all those layers.
   * This class is not meant to be instantiated, hence the protected constructor
   */
  class TextBasedProtocolMessage : public AbstractLayer {
  ...
  public:
    ...
    /**
     * @return A pointer to the first common field exists in this message, or NULL if no such field exists
     */
    ...
    /**
     * @return The number of common fields currently in the layer (not including CRLF at the end of the common)
     */
    int getFieldCount() const;

    /**
     * Add a new common field to this message. This field will be added last (before the end-of-common field)
     * @param[in] fieldName The field name
     * @param[in] fieldValue The field value
     * @return A pointer to the newly created common field, or NULL if the field could not be created
     */
    virtual HeaderField *addField(const std::string &fieldName, const std::string &fieldValue);

    /**
     * Add a new common field to this message. This field will be added last (before the end-of-common field)
     * @param[in] newField The common field to add
     * @return A pointer to the newly created common field, or NULL if the field could not be created
     */
    virtual HeaderField *addField(const HeaderField &newField);

    /**
     * Add the special end-of-common field (see the explanation in HeaderField)
     * @return A pointer to the newly created common field, or NULL if the field could not be created
     */
     ...
     */
    virtual HeaderField *
    insertField(std::string prevFieldName, const std::string &fieldName, const std::string &fieldValue);
    ...
    /**
     * Indicate whether the common is complete (ending with end-of-common "\r\n\r\n" or "\n\n") or spread over more packets
     * @return True if the common is complete or false if not
     */
