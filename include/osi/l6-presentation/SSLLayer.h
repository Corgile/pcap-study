...
namespace pcpp {
   ...
   */
  class SSLLayer : public AbstractLayer {
  public:
     ...
     */
    static bool
    IsSSLMessage(uint16_t srcPort, uint16_t dstPort, uint8_t *data, size_t dataLen, bool ignorePorts = false);
     ...
     * NULL is returned
     */
    static SSLLayer *createSSLMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * Get a pointer to the record common. Notice this points directly to the data, so every change will change the actual packet data
  ...
  public:
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    SSLHandshakeLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
