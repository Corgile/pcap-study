...
namespace pcpp {
  ...
  private:
    ...
    AbstractLayer *m_FirstLayer;
    AbstractLayer *m_LastLayer;
  ...
  public:
     ...
     * such cases)
     */
    bool addLayer(AbstractLayer *newLayer, bool ownInPacket = false) { return insertLayer(m_LastLayer, newLayer, ownInPacket); }
     ...
     * such cases)
     */
    bool insertLayer(AbstractLayer *prevLayer, AbstractLayer *newLayer, bool ownInPacket = false);
     ...
     */
    bool removeAllLayersAfter(AbstractLayer *layer);
     ...
     */
    AbstractLayer *detachLayer(ProtocolType layerType, int index = 0);
     ...
     * appropriate error log message will be printed
     */
    bool detachLayer(AbstractLayer *layer) { return removeLayer(layer, false); }
     ...
     */
    AbstractLayer *getLayerOfType(ProtocolType layerType, int index = 0) const;
    ...
    /**
     * Each layer can have fields that can be calculate automatically from other fields using AbstractLayer#computeCalculateFields(). This method forces all layers to calculate these
     * fields values
     */
    void computeCalculateFields();

    /**
     * Each layer can print a string representation of the layer most important data using AbstractLayer#toString(). This method aggregates this string from all layers and
  ...
  private:
    ...
    bool extendLayer(AbstractLayer *layer, int offsetInLayer, size_t numOfBytesToExtend);

    bool shortenLayer(AbstractLayer *layer, int offsetInLayer, size_t numOfBytesToShorten);

    void reallocateRawData(size_t newSize);

    bool removeLayer(AbstractLayer *layer, bool tryToDelete);

    std::string printPacketInfo(bool timeAsLocalTime) const;

    AbstractLayer *createFirstLayer(LinkLayerType linkType);
  }; // class Packet
  ...
  template<class TLayer>
  TLayer *Packet::getPrevLayerOfType(AbstractLayer *curLayer) const {
    if (curLayer == nullptr)
      return nullptr;
