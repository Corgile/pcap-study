...
#define PCPP_HTTP_SERVER_FIELD        "Server"
  ...
  /**
   * @class HttpRequestLayer
   * Represents an HTTP request common and inherits all basic functionality of HttpMessage and TextBasedProtocolMessage.
   ...
   * Unlike most L2-4 protocols, an HTTP request common can spread over more than 1 packet. PcapPlusPlus currently doesn't support a common
   ...
   * won't be recognized as HttpRequestLayer) and 2) the HTTP common for the first packet won't be complete (as it continues in the following
   * packets), this why PcapPlusPlus can indicate that HTTP request common is complete or not(doesn't end with "\r\n\r\n" or "\n\n") using
  ...
  public:
    ...
    */
    HttpRequestLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * A constructor that allocates a new HTTP request common with only the first line filled. Object will be created without further fields.
  ...
  /**
   * @class HttpResponseLayer
   * Represents an HTTP response common and inherits all basic functionality of HttpMessage and TextBasedProtocolMessage.
   ...
   * Unlike most L2-4 protocols, an HTTP response common can spread over more than 1 packet. PcapPlusPlus currently doesn't support a common
   ...
   * won't be recognized as HttpResponseLayer) and 2) the HTTP common for the first packet won't be complete (as it continues in the following
   * packets), this why PcapPlusPlus can indicate that HTTP response common is complete or not (doesn't end with "\r\n\r\n" or "\n\n") using
   * HttpMessage#isHeaderComplete()
   */
  ...
  public:
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    HttpResponseLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * A constructor that allocates a new HTTP response common with only the first line filled. Object will be created without further fields.
     * The user can then add fields using addField() methods
    ...
    /**
     * The length of the body of many HTTP response messages is determined by a HTTP common field called "Content-Length". This method sets
    ...
    /**
     * The length of the body of many HTTP response messages is determined by a HTTP common field called "Content-Length". This method
  ...
  /**
   * @class HttpRequestFirstLine
   * Represents an HTTP request common first line. The first line includes 3 parameters: HTTP method (e.g GET, POST, etc.),
  ...
  public:
    ...
    /**
     * As explained in HttpRequestLayer, an HTTP common can spread over more than 1 packet, so when looking at a single packet
  ...
  /**
   * @class HttpResponseFirstLine
   * Represents an HTTP response common first line. The first line includes 2 parameters: status code (e.g 200 OK, 404 Not Found, etc.),
  ...
  public:
    ...
    /**
     * As explained in HttpResponseLayer, an HTTP common can spread over more than 1 packet, so when looking at a single packet
     * the common can be partial. Same goes for the first line - it can spread over more than 1 packet. This method returns an indication
     * whether the first line is partial
     * @return False if the first line is partial, true if it's complete
     */
