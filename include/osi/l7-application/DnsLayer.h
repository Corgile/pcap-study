...
namespace pcpp {
  ...
  /**
   * @struct dnshdr
   * Represents the fixed part of the DNS common, meaning the part that doesn't include the DNS data (queries, answers, authorities
...
#pragma pack(pop)
   ...
   */
  class DnsLayer : public AbstractLayer {
  ...
  public:
     ...
     */
    DnsLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
     ...
     */
    DnsResource *
    addAnswer(const std::string &name, DnsType dnsType, DnsClass dnsClass, uint32_t ttl, IDnsResourceData *data);
     ...
     */
    DnsResource *
    addAuthority(const std::string &name, DnsType dnsType, DnsClass dnsClass, uint32_t ttl, IDnsResourceData *data);
     ...
     */
    DnsResource *
    addAdditionalRecord(const std::string &name, DnsType dnsType, uint16_t customData1, uint32_t customData2,
                        IDnsResourceData *data);
  ...
  protected:
    DnsLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, size_t offsetAdjustment);
  ...
  private:
    ...
    using AbstractLayer::extendLayer;

    bool extendLayer(int offsetInLayer, size_t numOfBytesToExtend, IDnsResource *resource);

    using AbstractLayer::shortenLayer;

    bool shortenLayer(int offsetInLayer, size_t numOfBytesToShorten, IDnsResource *resource);

    IDnsResource *
    getResourceByName(IDnsResource *startFrom, size_t resourceCount, const std::string &name, bool exactMatch) const;
  ...
  public:
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    DnsOverTcpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
