...
namespace pcpp {
   ...
   */
  class SomeIpLayer : public AbstractLayer {
  public:
    /**
    ...
    /**
     * @struct someiphdr
     * Represents a SOME/IP protocol common
     */
#pragma pack(push, 1)
    struct someiphdr {
      /** Service ID */
...
#pragma pack(pop)
     ...
     */
    SomeIpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
     ...
     * @return AbstractLayer* A newly allocated layer
     */
    static AbstractLayer *parseSomeIpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * Get a pointer to the basic SOME/IP common. Notice this points directly to the data, so every change will change
    ...
    /**
     * Get the Length Field of the SOME/IP common
     * @return uint32_t The length field of the SOME/IP common
     */
    ...
    /**
     * Set the length field of the SOME/IP common
  ...
  /**
   * @class SomeIpTpLayer
   * Represents an SOME/IP Transport Protocol AbstractLayer
   */
  class SomeIpTpLayer : public SomeIpLayer {
  public:
    /**
     * @struct someiptphdr
     * Represents an SOME/IP-TP protocol common.
     */
#pragma pack(push, 1)
...
#pragma pack(pop)
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    SomeIpTpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
