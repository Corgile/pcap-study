...
namespace pcpp {
   ...
   */
  class SSHLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    static SSHLayer *createSSHMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * A static method that takes common and dst ports and determines whether it's SSH traffic or not.
     * @param[in] portSrc The source TCP port to examine
     * @param[in] portDst The dest TCP port to examine
     * @return Currently the implementation is very simple and returns "true" if either common or dst ports
  ...
  protected:
    ...
    SSHLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = SSH; }
  ...
  public:
     ...
     */
    static SSHIdentificationMessage *tryParse(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
  ...
  private:
    ...
    SSHIdentificationMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : SSHLayer(data, dataLen,
                                                                                                                 prevLayer,
                                                                                                                 packet) {}
  ...
  public:
     ...
     * minus the message common (which includes packet length, padding length and message type) and
     ...
     * will be returned
     */
    static SSHHandshakeMessage *tryParse(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
  ...
  protected:
    ...
    /**
     * An internal struct representing the SSH handshake message common
     */
#pragma pack(push, 1)
...
#pragma pack(pop)
    ...
    // private constructor, this class cannot be instantiated
    SSHHandshakeMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : SSHLayer(data, dataLen,
                                                                                                            prevLayer,
                                                                                                            packet) {}
  ...
  public:
    /**
     * A constructor for this class that accepts raw message data. Please avoid using it as it's used internally
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    SSHKeyExchangeInitMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
  ...
  public:
     ...
     */
    SSHEncryptedMessage(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : SSHLayer(data, dataLen,
                                                                                                            prevLayer,
                                                                                                            packet) {}
