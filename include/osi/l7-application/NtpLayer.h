 ...
 * Represents a NTP (Network Time Protocol) layer
   ...
   * The packet format consists of three components: the common itself, one or more optional extension fields (for v4),
   * and an optional message authentication code (MAC). Currently the extension fields are not supported. The NTP common is:
   *
   * @verbatim
     0                   1                   2                   3
   ...
   *
 */
  class NtpLayer : public AbstractLayer {
  private:
#pragma pack(push, 1)
    struct ntp_header {
 ...
 */
    NtpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = NTP; }
