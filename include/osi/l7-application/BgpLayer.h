...
namespace pcpp {
   ...
   */
  class BgpLayer : public AbstractLayer {
  public:
...
#pragma pack(push, 1)
    struct bgp_common_header {
      /** 16-octet marker */
      uint8_t marker[16];
      /** Total length of the message, including the common */
      uint16_t length;
      /** BGP message type */
      uint8_t messageType;
    };
#pragma pack(pop)
     ...
     */
    static BgpLayer *parseBgpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
  ...
  protected:
    ...
    BgpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = BGP; }
...
#pragma pack(pop)
     ...
     */
    BgpOpenMessageLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : BgpLayer(data, dataLen,
                                                                                                            prevLayer,
                                                                                                            packet) {}

    /**
     * A constructor that creates a new BGP OPEN message
  ...
  public:
     ...
     * It's used to represent BGP Withdrawn Routes and Network AbstractLayer Reachability Information (NLRI)
     */
    struct prefix_and_ip {
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    BgpUpdateMessageLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : BgpLayer(data, dataLen,
                                                                                                              prevLayer,
                                                                                                              packet) {}

    /**
     * A constructor that creates a new BGP UPDATE message
    ...
    /**
     * @return The size in [bytes] of the Network AbstractLayer Reachability Info
     */
    size_t getNetworkLayerReachabilityInfoLength() const;

    /**
     * Get a vector of the Network AbstractLayer Reachability Info currently in the message
...
#pragma pack(pop)
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    BgpNotificationMessageLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : BgpLayer(data,
                                                                                                                    dataLen,
                                                                                                                    prevLayer,
                                                                                                                    packet) {}

    /**
     * A constructor that creates a new BGP NOTIFICATION message
  ...
  public:
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    BgpKeepaliveMessageLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : BgpLayer(data, dataLen,
                                                                                                                 prevLayer,
                                                                                                                 packet) {}

    /**
     * A constructor that creates a new BGP KEEPALIVE message
     */
...
#pragma pack(pop)
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    BgpRouteRefreshMessageLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : BgpLayer(data,
                                                                                                                    dataLen,
                                                                                                                    prevLayer,
                                                                                                                    packet) {}

    /**
     * A constructor that creates a new BGP ROUTE-REFRESH message
