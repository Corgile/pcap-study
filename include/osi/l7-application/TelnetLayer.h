...
namespace pcpp {
  ...
  /**
   * Class for representing the Telnet AbstractLayer
   */
  class TelnetLayer : public AbstractLayer {
  private:
  ...
  public:
     ...
     */
    TelnetLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                         packet) {
      m_Protocol = Telnet;
    ...
    /**
     * @return The OSI layer level of Telnet (Application AbstractLayer).
     */
