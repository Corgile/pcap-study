...
#define PCPP_SDP_MEDIA_NAME_FIELD       "m"
  ...
  public:
    ...
    */
    SdpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * An empty constructor which initialize an empty message with no fields
     */
    SdpLayer();

    /**
     * A constructor which initializes a message with the minimum required fields.<BR>
     * After this constructor the message will look like this:
     *
     * @code
     * v=0
    ...
    bool addMediaDescription(const std::string &mediaType, uint16_t mediaPort, const std::string &mediaProtocol,
                             const std::string &mediaFormat, std::vector<std::string> mediaAttributes);
