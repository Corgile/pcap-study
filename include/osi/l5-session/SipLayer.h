...
#include "osi/l6-presentation/TextBasedProtocol.h"
 ...
 */
namespace pcpp {
  // some popular SIP common fields
...
#define PCPP_SIP_RECORD_ROUTE_FIELD        "Record-Route"
  ...
  public:
    ...
    /**
     * The length of the body of many SIP response messages is determined by a SIP common field called "Content-Length". This method
    ...
    /**
     * The length of the body of many SIP messages is determined by a common field called "Content-Length". This method sets
  ...
  protected:
    SipLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : TextBasedProtocolMessage(data, dataLen,
                                                                                                                 prevLayer,
                                                                                                                 packet) {}
   ...
   * Represents a SIP request common and inherits all basic functionality of SipLayer and TextBasedProtocolMessage.
   ...
   * Since it's not an "ordinary" common field, it requires a special treatment and gets a class of it's own: SipRequestFirstLine.
   ...
   * request layer. You can find out whether the common is complete by using SipLayer#isHeaderComplete()
   */
  ...
  public:
    ...
    */
    SipRequestLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
   ...
   * <i>200 OK SIP/2.0</i>
   * Since it's not an "ordinary" common field, it requires a special treatment and gets a class of it's own: SipResponseFirstLine.
   ...
   * response layer. You can find out whether the common is complete by using SipLayer#isHeaderComplete()
   */
  ...
  public:
    ...
    enum SipResponseStatusCode {
      ...
      /** The original Request-URI is no longer valid, the new address is given in the Contact common field, and the client should update any records of the original Request-URI with the new value */
      ...
      /** The resource identified by the request is only capable of generating response entities that have content characteristics but not acceptable according to the Accept common field sent in the request */
      Sip406NotAcceptable,
      ...
      /** There was a resource-priority option tag, but no Resource-Priority common */
      Sip417UnknownResourcePriority,
      /** Bad SIP Protocol Extension used, not understood by the server */
      Sip420BadExtension,
      /** The server needs a specific extension not listed in the Supported common */
      Sip421ExtensionRequired,
      /** The received request contains a Session-Expires common field with a duration below the minimum timer */
      ...
      /** The server policy requires an Identity common, and one has not been provided */
      ...
      /** The request has an Identity-Info common, and the URI scheme in that common cannot be dereferenced */
      ...
      /** If a SIP UA receives an INFO request associated with an Info Package that the UA has not indicated willingness to receive, the UA MUST send a 469 response, which contains a Recv-Info common field with Info Packages for which the UA is willing to receive INFO requests */
      ...
      Sip482LoopDetected,
      /** Max-Forwards common has reached the value '0' */
      Sip483TooManyHops,
      ...
      /** The server did not understand an event package specified in an Event common field */
      Sip489BadEvent,
      ...
      Sip502BadGateway,
      /** The server is undergoing maintenance or is temporarily overloaded and so cannot process the request. A "Retry-After" common field may specify when the client may reattempt its request */
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    SipResponseLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
     ...
     * But the user can set a non-default status code string and it will be written in the common first line. Empty string ("") means using the
  ...
  public:
     ...
     * the common can be partial. Same goes for the first line - it can spread over more than 1 packet. This method returns an indication
