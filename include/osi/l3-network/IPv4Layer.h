...
namespace pcpp {
  ...
  /**
   * @struct iphdr
   * Represents an IPv4 protocol common
   */
#pragma pack(push, 1)
  struct iphdr {
#if (BYTE_ORDER == LITTLE_ENDIAN)
    /** IP common length, has the value of 5 for IPv4 */
    ...
    /** Entire packet (fragment) size, including common and data, in bytes */
...
#define PCPP_IP_MORE_FRAGMENTS 0x20
  ...
  struct IPv4TimestampOptionValue {
    ...
    /** A list of timestamps parsed from the sf_IPv4 timestamp option value */
    std::vector<uint32_t> timestamps;

    /** A list of sf_IPv4 addresses parsed from the sf_IPv4 timestamp option value */
  ...
  /**
   * @class IPv4Option
   * A wrapper class for sf_IPv4 options. This class does not create or modify sf_IPv4 option records, but rather
  ...
  public:
    ...
    /**
     * A constructor for this class that gets a pointer to the option raw data (byte array)
     * @param[in] optionRawData A pointer to the sf_IPv4 option raw data
     */
     ...
     * such as ::IPV4OPT_RecordRoute, ::IPV4OPT_StrictSourceRoute, ::IPV4OPT_LooseSourceRoute, etc. This method returns a vector of the sf_IPv4 addresses. Blank IP addresses
     ...
     * @return A vector of sf_IPv4 addresses parsed from the sf_IPv4 option value
     */
    ...
    /**
     * A method for parsing the sf_IPv4 timestamp option value. This method is relevant only for sf_IPv4 timestamp option. For other option types an empty result will be returned.
     ...
     * @return A structured containing the sf_IPv4 timestamp value
     */
    ...
    /**
     * @return sf_IPv4 option type casted as pcpp::IPv4OptionTypes enum
     */
   ...
   * A class for building sf_IPv4 option records. This builder receives the sf_IPv4 option parameters in its constructor,
   * builds the sf_IPv4 option raw buffer and provides a build() method to get a IPv4Option object out of it
   */
  ...
  public:
    ...
    /**
     * A constructor for building sf_IPv4 options which their value is a byte array. The IPv4Option object can be later
     * retrieved by calling build()
     * @param[in] optionType sf_IPv4 option type
    ...
    /**
     * A constructor for building sf_IPv4 options which have a 2-byte value. The IPv4Option object can be later retrieved
     * by calling build()
     * @param[in] optionType sf_IPv4 option type
    ...
    /**
     * A constructor for building sf_IPv4 options which their value is a list of sf_IPv4 addresses, for example:
     * ::IPV4OPT_RecordRoute, ::IPV4OPT_StrictSourceRoute, ::IPV4OPT_LooseSourceRoute. The IPv4Option object can be later retrieved
     * by calling build()
     * @param[in] optionType sf_IPv4 option type
     * @param[in] ipList A vector of sf_IPv4 addresses that will be used as the option value
     */
    IPv4OptionBuilder(IPv4OptionTypes optionType, const std::vector<IPv4Address> &ipList);

    /**
     * A constructor for building sf_IPv4 timestamp option (::IPV4OPT_Timestamp). The IPv4Option object can be later retrieved by calling build()
     * @param[in] timestampValue The timestamp value to build the sf_IPv4 option with
     */
    explicit IPv4OptionBuilder(const IPv4TimestampOptionValue &timestampValue);

    /**
     * Build the IPv4Option object out of the parameters defined in the constructor
  ...
  /**
   * @class IPv4Layer
   * Represents an sf_IPv4 protocol layer
   */
  class IPv4Layer : public AbstractLayer, public IPLayer {
  public:
    /**
     ...
     */
    IPv4Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
     ...
     * @param[in] prevLayer A pointer to the previous layer
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     * @param[in] setTotalLenAsDataLen When setting this value to "true" or when using the other constructor, the layer data length is calculated
     ...
     */
    IPv4Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, bool setTotalLenAsDataLen);


    /**
     * A constructor that allocates a new sf_IPv4 common with empty fields
     */
    IPv4Layer();

    /**
     * A constructor that allocates a new sf_IPv4 common with source and destination sf_IPv4 addresses
     * @param[in] srcIP Source sf_IPv4 address
     * @param[in] dstIP Destination sf_IPv4 address
     */
    IPv4Layer(const IPv4Address &srcIP, const IPv4Address &dstIP);

    /**
     * A copy constructor that copy the entire common from the other IPv4Layer (including sf_IPv4 options)
     */
    IPv4Layer(const IPv4Layer &other);

    /**
     * An assignment operator that first delete all data from current layer and then copy the entire common from the other IPv4Layer (including sf_IPv4 options)
     */
    IPv4Layer &operator=(const IPv4Layer &other);

    /**
     * Get a pointer to the sf_IPv4 common. Notice this points directly to the data, so every change will change the actual packet data
     ...
     * (which usually contains the L4 common). Return false otherwise (not a fragment or not the first fragment)
     */
    ...
    /**
     * Get an sf_IPv4 option by type.
     * @param[in] option sf_IPv4 option type
    ...
    /**
     * @return The first sf_IPv4 option in the packet. If the current layer contains no options the returned value will contain
     * a logical NULL (IPv4Option#isNull() == true)
     */
    IPv4Option getFirstOption() const;

    /**
     * Get the sf_IPv4 option that comes after a given option. If the given option was the last one, the
     * returned value will contain a logical NULL (IPv4Option#isNull() == true)
     * @param[in] option An sf_IPv4 option object that exists in the current layer
     * @return A IPv4Option object that contains the sf_IPv4 option data that comes next, or logical NULL if the given
     * sf_IPv4 option: (1) was the last one; or (2) contains a logical NULL; or (3) doesn't belong to this packet
     */
    IPv4Option getNextOption(IPv4Option &option) const;

    /**
     * @return The number of sf_IPv4 options in this layer
     */
    size_t getOptionCount() const;

    /**
     * Add a new sf_IPv4 option at the end of the layer (after the last sf_IPv4 option)
     * @param[in] optionBuilder An IPv4OptionBuilder object that contains the sf_IPv4 option data to be added
     * @return A IPv4Option object that contains the newly added sf_IPv4 option data or logical NULL
    ...
    /**
     * Add a new sf_IPv4 option after an existing one
     * @param[in] optionBuilder An IPv4OptionBuilder object that contains the requested sf_IPv4 option data to be added
     * @param[in] prevOptionType The sf_IPv4 option which the newly added option should come after. This is an optional parameter which
     ...
     * @return A IPv4Option object containing the newly added sf_IPv4 option data or logical NULL
     * (IPv4Option#isNull() == true) if addition failed. In case of a failure a corresponding error message will be
     * printed to log
     */
    ...
    /**
     * Remove all sf_IPv4 options from the layer
     ...
     * @return True if the data is valid and can represent an sf_IPv4 packet
     */
