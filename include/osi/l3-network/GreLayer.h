...
namespace pcpp {
   ...
   * Represents GRE basic protocol common (common for GREv0 and GREv1)
   */
#pragma pack(push, 1)
  struct gre_basic_header {
...
#pragma pack(pop)
  ...
  /**
   * @struct gre1_header
   * Represents GREv1 protocol common
   */
#pragma pack(push, 1)
  struct gre1_header : gre_basic_header {
    /** Size of the payload not including the GRE common */
...
#pragma pack(pop)
  ...
  public:
    ...
    */
    GREv0Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : GreLayer(data, dataLen, prevLayer,
                                                                                                   packet) { m_Protocol = GREv0; }

    /**
     * A constructor that creates a new GREv0 common and allocates the data
     */
    GREv0Layer();
     ...
     * as setChecksum(), unsetChecksum(), etc.) may result to wrong calculation of common length and really weird bugs.
  ...
  public:
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    GREv1Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : GreLayer(data, dataLen, prevLayer,
                                                                                                   packet) { m_Protocol = GREv1; }

    /**
     * A constructor that creates a new GREv1 common and allocates the data
     ...
     * (such as setAcknowledgmentNum(), unsetSequenceNumber(), etc.) may result to wrong calculation of common length or illegal GREv1 packet and
  ...
  /**
   * @class PPP_PPTPLayer
   * Represent a PPP (point-to-point) protocol common that comes after GREv1 common, as part of PPTP - Point-to-Point Tunneling Protocol
   */
  class PPP_PPTPLayer : public AbstractLayer {
  public:
    /**
     * A constructor that creates the layer from an existing packet raw data
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    PPP_PPTPLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                           packet) { m_Protocol = PPP_PPTP; }

    /**
     * A constructor that allocates a new PPP-PPTP common
