...
namespace pcpp {
  ...
  */
  ...
  struct vrrp_header {
     ...
     * VRRPv2 uses normal checksum algorithm, while VRRPv3 uses "pseudo-common" checksum algorithm. */
    uint16_t checksum;
   ...
   * VRRPv2 and VRRPv3
   */
  class VrrpLayer : public AbstractLayer {
  private:
  ...
  protected:
    VrrpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, ProtocolType vrrpVer,
              IPAddress::AddressType addressType)
        : AbstractLayer(data, dataLen, prevLayer, packet), m_AddressType(addressType) {
      m_Protocol = vrrpVer;
    }
  ...
  public:
    ...
    */
    ...
    /**
     * Fill the checksum from common and data and write the result to @ref vrrp_header#checksum
     */
    void calculateAndSetChecksum();

    /**
     * Calculate the checksum from common and data and write the result to @ref vrrp_header#checksum
    ...
    /**
     * @return The message size in bytes which include the size of the basic common + the size of the IP address(es)
     */
  ...
  public:
    ...
    */
    VrrpV2Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
  ...
  public:
    ...
    */
    VrrpV3Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, IPAddress::AddressType addressType)
