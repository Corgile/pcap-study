...
namespace pcpp {
  ...
  /**
   * @struct arphdr
   * Represents an ARP protocol common
   */
#pragma pack(push, 1)
  struct arphdr {
    /** Hardware type (HTYPE) */
...
#pragma pack(pop)
   ...
   */
  class ArpLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    ArpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) {
      m_Protocol = ARP;
    ...
    /**
     * A constructor that allocates a new ARP common
    ...
    /**
     * Get a pointer to the ARP common. Notice this points directly to the data, so every change will change the actual packet data
