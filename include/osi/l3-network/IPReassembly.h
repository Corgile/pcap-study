 ...
 * constructor). Once capacity (the number of concurrent reassembled packets) exceeds this number, the packet that was least recently used will be
 ...
 * reassembled packets are removed from the map by registering to the pcpp#IPReassembly#OnFragmentsClean callback in pcpp#IPReassembly constructor
 */
...
#define PCPP_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE 500000
  ...
  public:
    ...
    public:
      ...
      /**
       * A default constructor which zeros all members
       */
      IPv4PacketKey() : m_IpID(0), m_SrcIP(IPv4Address::Zero), m_DstIP(IPv4Address::Zero) {}

      /**
       * A constructor that sets values in each one of the members
       * @param[in] ipid IP ID value
       * @param[in] srcip Source IPv4 address
       * @param[in] dstip Dest IPv4 address
       */
      IPv4PacketKey(uint16_t ipid, IPv4Address srcip, IPv4Address dstip) : m_IpID(ipid), m_SrcIP(srcip),
                                                                           m_DstIP(dstip) {}
    ...
    public:
       ...
       */
      IPv6PacketKey(uint32_t fragmentID, IPv6Address srcip, IPv6Address dstip) : m_FragmentID(fragmentID),
                                                                                 m_SrcIP(srcip), m_DstIP(dstip) {}

      /**
       * A copy constructor for this class
       * @param[in] other The instance to copy from
       */
      IPv6PacketKey(const IPv6PacketKey &other) : PacketKey(other), m_FragmentID(other.m_FragmentID),
     ...
     * The IP reassembly mechanism has a certain capacity of concurrent packets it can handle. This capacity is determined in its constructor
     ...
     * @param[in] userCookie A pointer to the cookie provided by the user in IPReassemby constructor (or NULL if no cookie provided)
     */
    ...
    enum ReassemblyStatus {
      /** The processed packet isn't of type sf_IPv4 or IPv6 */
      NON_IP_PACKET = 0x00,
     ...
     * @param[in] fragment The fragment to process (sf_IPv4 or IPv6). Please notice that the reassembly logic doesn't change or manipulate
     ...
     * - The input fragment is not a sf_IPv4 or IPv6 packet
     * - The input fragment is not a sf_IPv4 or IPv6 fragment packet
     ...
     * certain layer and want to avoid the performance impact and memory consumption of parsing the whole packet. Note that setting this to a protocol which doesn't
         * include the IP-AbstractLayer will result in IPReassembly not finding the IP-AbstractLayer and thus failing to work properly. Default value is ::UnknownProtocol which means
     ...
     * @return
     * - If the input fragment isn't an sf_IPv4/IPv6 packet or if it isn't an sf_IPv4/IPv6 fragment, the return value is a pointer to the input fragment
    ...
    /**
     * The main API that drives IPReassembly. This method should be called whenever a fragment arrives. This method finds the relevant
     * packet this fragment belongs to and runs the sf_IPv4 reassembly logic that is described in IPReassembly.h.
     * @param[in] fragment The fragment to process (sf_IPv4 or IPv6). Please notice that the reassembly logic doesn't change or manipulate
     * this object in any way. All of its data is copied to internal structures and manipulated there
     * @param[out] status An indication of the packet reassembly status following the processing of this fragment. Possible values are:
     * - The input fragment is not a sf_IPv4 or IPv6 packet
     ...
     * Default value is ::OsiModelLayerUnknown which means don't take this parameter into account
     * @return
     * - If the input fragment isn't an sf_IPv4/IPv6 packet or if it isn't an sf_IPv4/IPv6 fragment, the return value is a pointer to a Packet object
    ...
    /**
     * Get the maximum capacity as determined in the constructor
     */
