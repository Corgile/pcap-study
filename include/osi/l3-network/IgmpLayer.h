...
namespace pcpp {
  ...
  /**
   * @struct igmp_header
   * IGMPv1 and IGMPv2 basic protocol common
   */
  struct igmp_header {
   ...
   * IGMPv3 membership query basic common
   */
  struct igmpv3_query_header {
   ...
   * IGMPv3 membership report basic common
   */
  struct igmpv3_report_header {
   ...
   */
  class IgmpLayer : public AbstractLayer {
  protected:

    IgmpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet, ProtocolType igmpVer) : AbstractLayer(data,
                                                                                                                             dataLen,
                                                                                                                             prevLayer,
                                                                                                                             packet) { m_Protocol = igmpVer; }
  ...
  public:
    ...
    */
    IgmpV1Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
        : IgmpLayer(data, dataLen, prevLayer, packet, IGMPv1) {}

    /**
     * A constructor that allocates a new IGMPv1 common
  ...
  public:
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    IgmpV2Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
        : IgmpLayer(data, dataLen, prevLayer, packet, IGMPv2) {}

    /**
     * A constructor that allocates a new IGMPv2 common
     * @param[in] type The message type to set
  ...
  public:
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    IgmpV3QueryLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);
    ...
    /**
     * Get a pointer to the raw IGMPv3 membership query common. Notice this points directly to the data, so every change will change the
     * actual packet data
  ...
  public:
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    IgmpV3ReportLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
    ...
    /**
     * Get a pointer to the raw IGMPv3 membership report common. Notice this points directly to the data, so every change will change the
     * actual packet data
