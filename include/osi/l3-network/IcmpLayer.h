...
namespace pcpp {
  ...
  /**
   * @struct icmphdr
   * Represents ICMP basic protocol common (common for all ICMP message types)
   */
#pragma pack(push, 1)
...
#pragma pack(push, 1)
  typedef struct icmp_param_problem : icmphdr {
    /** in the case of an invalid IP common (Code 0), this field indicates the byte offset of the error in the common */
    uint8_t pointer;
    /** unused 1 byte */
    uint8_t unused1;
    /** unused 2 bytes */
...
#pragma pack(push, 1)
  ...
  struct icmp_router_address_structure {
    /** the sf_IPv4 address of the advertised router */
    ...
    /**
     * Set router address structure from a given sf_IPv4 address and preference level
     * @param[in] addr sf_IPv4 address to set
     * @param[in] preference Preference level to set
     */
    void setRouterAddress(IPv4Address addr, uint32_t preference);

    /**
     * @return The sf_IPv4 address extracted from icmp_router_address_structure#routerAddress field
     */
...
#pragma pack(pop)
   ...
   */
  struct icmp_router_advertisement {
    /** a pointer to the common data on the packet */
...
#pragma pack(pop)
  ...
  /**
   * @class IcmpLayer
   * Represents an ICMP protocol layer (for sf_IPv4 only)
   */
  class IcmpLayer : public AbstractLayer {
  private:
    ...
    bool setEchoData(IcmpMessageType echoType, uint16_t id, uint16_t sequence, uint64_t timestamp, const uint8_t *data,
                     size_t dataLen);

    bool setIpAndL4Layers(IPv4Layer *ipLayer, AbstractLayer *l4Layer);
  ...
  public:
    ...
    IcmpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                       packet) { m_Protocol = ICMP; }

    /**
     * An empty constructor that creates a new layer with an empty ICMP common without setting the ICMP type or ICMP data.
     ...
     */
    icmp_echo_request *
    setEchoRequestData(uint16_t id, uint16_t sequence, uint64_t timestamp, const uint8_t *data, size_t dataLen);
     ...
     */
    icmp_echo_reply *
    setEchoReplyData(uint16_t id, uint16_t sequence, uint64_t timestamp, const uint8_t *data, size_t dataLen);
    ...
    icmp_timestamp_reply *setTimestampReplyData(uint16_t id, uint16_t sequence,
                                                timeval originateTimestamp, timeval receiveTimestamp,
     ...
     * The IP and L4 (ICMP/TCP/UDP) osi of the destination unreachable data are parsed as separate layers and can be
     ...
     * a standalone layer). The reason is the Internet and L4 osi given as parameters are added as separate layers
     ...
     * @param[in] ipHeader The Internet common of the original data. This layer is added as a separate layer on the packet
     * @param[in] l4Header The L4 common of the original data. This layer is added as a separate layer on the packet
     * @return A pointer to the destination unreachable data that have been set or NULL if something went wrong
     * (an appropriate error log is printed in such cases)
     */
    icmp_destination_unreachable *
    setDestUnreachableData(IcmpDestUnreachableCodes code, uint16_t nextHopMTU, IPv4Layer *ipHeader, AbstractLayer *l4Header);

    /**
     * @return ICMP source quench data. If the layer isn't of type ICMP source quench NULL is returned.
     * The IP and L4 (ICMP/TCP/UDP) osi of the source quench data are parsed as separate layers and can be
     * retrieved via this->getNextLayer()
     */
     ...
     * (an appropriate error log is printed in such cases)
     */
    icmp_source_quench *setSourceQuenchdata(IPv4Layer *ipHeader, AbstractLayer *l4Header);

    /**
     * @return ICMP redirect data. If the layer isn't of type ICMP redirect NULL is returned.
     * The IP and L4 (ICMP/TCP/UDP) osi of the redirect data are parsed as separate layers and can be
     * retrieved via this->getNextLayer()
     */
     ...
     * @param[in] gatewayAddress An sf_IPv4 address of the gateway to which the redirection should be sent
     * @param[in] ipHeader The Internet common of the original data. This layer is added as a separate layer on the packet
     * @param[in] l4Header The L4 common of the original data. This layer is added as a separate layer on the packet
     * @return A pointer to the redirect data that have been set or NULL if something went wrong
     * (an appropriate error log is printed in such cases)
     */
    icmp_redirect *setRedirectData(uint8_t code, IPv4Address gatewayAddress, IPv4Layer *ipHeader, AbstractLayer *l4Header);
     ...
     * (an appropriate error log is printed in such cases)
     */
    icmp_router_advertisement *setRouterAdvertisementData(uint8_t code, uint16_t lifetimeInSeconds,
     ...
     * The IP and L4 (ICMP/TCP/UDP) osi of the time exceeded data are parsed as separate layers and can be
     * retrieved via this->getNextLayer()
     */
     ...
     * (an appropriate error log is printed in such cases)
     */
    icmp_time_exceeded *setTimeExceededData(uint8_t code, IPv4Layer *ipHeader, AbstractLayer *l4Header);
     ...
     * @param[in] errorOctetPointer In the case of an invalid IP common (Code 0), indicate the byte offset of the error in the common
     * @param[in] ipHeader The Internet common of the original data. This layer is added as a separate layer on the packet
     * @param[in] l4Header The L4 common of the original data. This layer is added as a separate layer on the packet
     ...
     */
    icmp_param_problem *
    setParamProblemData(uint8_t code, uint8_t errorOctetPointer, IPv4Layer *ipHeader, AbstractLayer *l4Header);
