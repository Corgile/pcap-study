...
namespace pcpp {
   ...
   * A class for building NDP option records. This builder receives the NDP option parameters in its constructor,
  ...
  public:
    /**
     * A constructor for building NDP options which their value is a byte array. The NdpOption object can be later
    ...
    /**
     * Build the NdpOption object out of the parameters defined in the constructor. Padding bytes are added to the
  ...
  protected:
    ...
    NDPLayerBase(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
...
#pragma pack(pop)
     ...
     */
    NDPNeighborSolicitationLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
    ...
    /**
     * Get the Link AbstractLayer Address
...
#pragma pack(pop)
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    NDPNeighborAdvertisementLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
        : NDPLayerBase(data, dataLen, prevLayer, packet) {}

    /**
     * A constructor that allocates a new NDP Advertisement AbstractLayer with target link-layer address option
     * @param[in] code Code field
    ...
    /**
     * A constructor that allocates a new NDP Advertisement AbstractLayer
