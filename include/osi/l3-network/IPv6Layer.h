...
namespace pcpp {
  ...
  /**
   * @struct ip6_hdr
   * Represents IPv6 protocol common
   */
#pragma pack(push, 1)
  struct ip6_hdr {
    ...
    /** Flow label */
    uint8_t flowLabel[3];
    /** The size of the payload in octets, including any extension osi */
    uint16_t payloadLength;
    /** Specifies the type of the next common (protocol). Must be one of ::IPProtocolTypes */
    uint8_t nextHeader;
...
#pragma pack(pop)
   ...
   */
  class IPv6Layer : public AbstractLayer, public IPLayer {
  public:
    /**
     ...
     */
    IPv6Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * A constructor that allocates a new IPv6 common with empty fields
     */
    IPv6Layer();

    /**
     * A constructor that allocates a new IPv6 common with source and destination IPv6 addresses
    ...
    /**
     * A copy constructor that copies the entire common from the other IPv6Layer (including IPv6 extensions)
     */
    ...
    /**
     * Get a pointer to the IPv6 common. Notice this points directly to the data, so every change will change the actual packet data
     ...
     * - ip6_hdr#payloadLength = size of payload (all data minus common size)
     * - ip6_hdr#ipVersion = 6
