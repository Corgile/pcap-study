...
namespace pcpp {
   ...
   * (has private constructor and copy constructor)
   */
  class IPv6Extension {
  ...
  public:
    ...
    /**
     * @return A pointer to the next common or NULL if the extension is the last one
     */
   ...
   * Represents an IPv6 fragmentation extension common and allows easy access to all fragmentation parameters
   */
  ...
  public:
     ...
     * A struct representing IPv6 fragmentation common
     */
    struct ipv6_frag_header {
      /** Next common type */
      uint8_t nextHeader;
      /** Fragmentation common size is fixed 8 bytes, so len is always zero */
      uint8_t headerLen;
    ...
    /**
     * A constructor for creating a new IPv6 fragmentation extension object not bounded to a packet. Useful for adding new extensions to an
    ...
    /**
     * Get a pointer to the fragmentation common. Notice the returned pointer points directly to the data, so every change will modify
    ...
    /**
     * @return True if this is the first fragment (which usually contains the L4 common), false otherwise
     */
  ...
  public:
     ...
     * A class for building IPv6 Type-Length-Value (TLV) options. This builder receives the option parameters in its constructor,
    ...
    public:
      ...
      /**
       * A constructor for building IPv6 TLV options which their value is a byte array. The IPv6Option object can later
      ...
      /**
       * A constructor for building IPv6 TLV options which have a 1-byte value. The IPv6Option object can later be retrieved
       * by calling build()
       * @param[in] optType IPv6 option type
      ...
      /**
       * A constructor for building IPv6 TLV options which have a 2-byte value. The IPv6Option object can later be retrieved
       * by calling build()
       * @param[in] optType IPv6 option type
      ...
      /**
       * Build the IPv6Option object out of the parameters defined in the constructor
  ...
  protected:
    ...
    /** A private constructor to keep this object from being constructed */
   ...
   * Represents IPv6 Hop-By-Hop extension common and allows easy access to all of its data including the TLV options stored
   */
  ...
  public:
    ...
    /**
     * A constructor for creating a new IPv6 Hop-By-Hop extension object not bounded to a packet. Useful for adding new extensions to an
     * IPv6 layer with IPv6Layer#addExtension()
     ...
     */
    explicit IPv6HopByHopHeader(const std::vector<IPv6TLVOptionBuilder> &options) : IPv6TLVOptionHeader(
        options) { m_ExtType = IPv6HopByHop; }

  private:

    IPv6HopByHopHeader(IDataContainer *dataContainer, size_t offset) : IPv6TLVOptionHeader(dataContainer,
                                                                                           offset) { m_ExtType = IPv6HopByHop; }
  };


  /**
   * @class IPv6DestinationHeader
   * Represents IPv6 destination extension common and allows easy access to all of its data including the TLV options stored in it
   */
  ...
  public:
    ...
    /**
     * A constructor for creating a new IPv6 destination extension object not bounded to a packet. Useful for adding new extensions to an
     * IPv6 layer with IPv6Layer#addExtension()
     * @param[in] options A vector of IPv6TLVOptionHeader#TLVOptionBuilder instances which define the options that will be stored in the
     * extension data. Notice this vector is read-only and its content won't be modified
     */
    explicit IPv6DestinationHeader(const std::vector<IPv6TLVOptionBuilder> &options) : IPv6TLVOptionHeader(
        options) { m_ExtType = IPv6Destination; }

  private:

    IPv6DestinationHeader(IDataContainer *dataContainer, size_t offset) : IPv6TLVOptionHeader(dataContainer,
                                                                                              offset) { m_ExtType = IPv6Destination; }
  };


  /**
   * @class IPv6RoutingHeader
   * Represents IPv6 routing extension common and allows easy access to all of its data
   */
  ...
  public:
     ...
     * A struct representing the fixed part of the IPv6 routing extension common
     */
    struct ipv6_routing_header {
      /** Next common type */
      uint8_t nextHeader;
      /** The length of this common, in multiples of 8 octets, not including the first 8 octets */
      uint8_t headerLen;
    ...
    /**
     * A constructor for creating a new IPv6 routing extension object not bounded to a packet. Useful for adding new extensions to an
     * IPv6 layer with IPv6Layer#addExtension()
    ...
    /**
     * Get a pointer to the fixed part of the routing common. Notice the return pointer points directly to the data, so every change will modify
     * the actual packet data
  ...
  private:
    ...
    IPv6RoutingHeader(IDataContainer *dataContainer, size_t offset) : IPv6Extension(dataContainer,
                                                                                    offset) { m_ExtType = IPv6Routing; }
   ...
   * Represents IPv6 authentication common extension (used in IPSec protocol) and allows easy access to all of its data
   */
  ...
  public:
     ...
     * A struct representing the fixed part of the IPv6 authentication common extension
     */
    struct ipv6_authentication_header {
      /** Next common type */
      uint8_t nextHeader;
    ...
    /**
     * A constructor for creating a new IPv6 authentication common extension object not bounded to a packet. Useful for adding new extensions to an
     * IPv6 layer with IPv6Layer#addExtension()
    ...
    /**
     * Get a pointer to the fixed part of the authentication common. Notice the return pointer points directly to the data, so every change
    ...
    /**
     * In the authentication common the extension length is calculated in a different way than other extensions. The
  ...
  private:
    ...
    IPv6AuthenticationHeader(IDataContainer *dataContainer, size_t offset) : IPv6Extension(dataContainer,
                                                                                           offset) { m_ExtType = IPv6AuthenticationHdr; }
  };
