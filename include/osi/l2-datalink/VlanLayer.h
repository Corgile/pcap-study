...
#pragma pack(pop)
   ...
   */
  class VlanLayer : public AbstractLayer {
  public:
    ...
    */
    VlanLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                       packet) { m_Protocol = VLAN; }

    /**
     * A constructor that allocates a new VLAN common
    ...
    /**
     * Get a pointer to the VLAN common. Notice this points directly to the data, so every change will change the actual packet data
     ...
     */
    size_t getHeaderLen() const override { return sizeof(vlan_header); }
