...
namespace pcpp {
  ...
  /**
   * @struct ether_header
   * Represents an Ethernet II common
   */
#pragma pack(push, 1)
  struct ether_header {
    /** Destination MAC */
...
#define PCPP_ETHERTYPE_WAKE_ON_LAN   0x0842
   ...
   */
  class EthLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    EthLayer(uint8_t *data, size_t dataLen, Packet *packet) : AbstractLayer(data, dataLen, NULL,
                                                                            packet) { m_Protocol = Ethernet; }
     ...
     * @param[in] packet A pointer to the Packet instance where layer will be stored in
     */
    EthLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = Ethernet; }

    /**
     * A constructor that creates a new Ethernet common and allocates the data
