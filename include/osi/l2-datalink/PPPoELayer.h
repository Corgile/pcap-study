...
namespace pcpp {
  ...
  /**
   * @struct pppoe_header
   * Represents an PPPoE protocol common
   */
#pragma pack(push, 1)
  struct pppoe_header {
    ...
    uint16_t sessionId;
    /** Length (in bytes) of payload, not including the PPPoE common */
...
#pragma pack(pop)
   ...
   * and PPPoE discovery
   */
  class PPPoELayer : public AbstractLayer {
  public:
    /**
  ...
  public:
    ...
    /**
     * A constructor that allocates a new PPPoE Session common with version, type and session ID
     * @param[in] version PPPoE version
     * @param[in] type PPPoE type
     * @param[in] sessionId PPPoE session ID
     * @param[in] pppNextProtocol The next protocol to come after the PPPoE session common. Should be one of the PPP_* macros listed below
     */
    PPPoESessionLayer(uint8_t version, uint8_t type, uint16_t sessionId, uint16_t pppNextProtocol) : PPPoELayer(version,
                                                                                                                type,
    ...
    /**
     * Set the field that describes which common comes after the PPPoE session common
  ...
  public:
    ...
    public:
      /**
       * A constructor that gets a pointer to the tag raw data (byte array)
     ...
     * A class for building PPPoE Tags. This builder receives the tag parameters in its constructor,
    ...
    public:
      ...
      /**
       * A constructor for building a PPPoE Tag which has no value (tag len is zero). The PPPoETag object can later
      ...
      /**
       * A constructor for building a PPPoE Tag which has some arbitrary value. The PPPoETag object can later
       * be retrieved by calling build()
       * @param[in] tagType Tag type
      ...
      /**
       * Build the PPPoETag object out of the parameters defined in the constructor
    ...
    /**
     * A constructor that allocates a new PPPoE Discovery common with version, type, PPPoE code and session ID
     * @param[in] version PPPoE version
     * @param[in] type PPPoE type
     * @param[in] code PPPoE code enum
     * @param[in] sessionId PPPoE session ID
     */
    PPPoEDiscoveryLayer(uint8_t version, uint8_t type, PPPoELayer::PPPoECode code, uint16_t sessionId) : PPPoELayer(
        version, type, code, sessionId) { m_Protocol = PPPoEDiscovery; }
...
#define PCPP_PPP_IPCP      0x8021
  /** OSI Network AbstractLayer Control Protocol */
