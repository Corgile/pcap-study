...
namespace pcpp {
   ...
   */
  class WakeOnLanLayer : public AbstractLayer {
  private:
...
#pragma pack(pop)
     ...
     */
    WakeOnLanLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
        : AbstractLayer(data, dataLen, prevLayer, packet) {
      m_Protocol = WakeOnLan;
    }

    /**
     * Construct a new Wake On Lan AbstractLayer with provided values
    ...
    /**
     * Get a pointer to the Wake On LAN common. Notice this points directly to the data, so every change will change
    ...
    /**
     * Set the password of the command
     * @param addr Password as sf_IPv4 address
     * @return True if operation successful, false otherwise
     */
