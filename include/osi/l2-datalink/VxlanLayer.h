...
namespace pcpp {
  ...
  /**
   * @struct vxlan_header
   * Represents a VXLAN protocol common
   */
#pragma pack(push, 1)
  struct vxlan_header {
...
#pragma pack(pop)
   ...
   */
  class VxlanLayer : public AbstractLayer {
  public:
    ...
    */
    VxlanLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                        packet) { m_Protocol = VXLAN; }

    /**
     * A constructor that creates a new VXLAN common and allocates the data. Note: the VNI present flag is set automatically
    ...
    /**
     * Get a pointer to the VXLAN common. Notice this points directly to the data, so every change will change the actual packet data
     ...
     */
    size_t getHeaderLen() const override { return sizeof(vxlan_header); }
