...
namespace pcpp {
   ...
   * An interface (virtual abstract class) that indicates an object that holds a pointer to a buffer data. The AbstractLayer class is an example
  ...
  /**
   * @class AbstractLayer
   * AbstractLayer is the base class for all protocol layers. Each protocol supported in PcapPlusPlus has a class that inherits AbstractLayer.
   * The protocol layer class expose all properties and methods relevant for viewing and editing protocol fields.
   * For example: a pointer to a structured common (e.g tcphdr, iphdr, etc.), protocol common size, payload size, compute
   ...
   * The AbstractLayer class, as a base class, is abstract and the user can't create an instance of it (it has a private constructor)
   ...
   *
  */
  class AbstractLayer : public IDataContainer {
    friend class Packet;
  ...
  public:
     ...
     */
    AbstractLayer *getNextLayer() const { return m_NextLayer; }

    /**
     * @return A pointer to the previous layer in the protocol stack or NULL if the layer is the first one
     */
    AbstractLayer *getPrevLayer() const { return m_PrevLayer; }
    ...
    /**
     * @return A pointer to the layer raw data. In most cases it'll be a pointer to the first byte of the common
     */
    uint8_t *getData() const { return m_Data; }

    /**
     * @return The length in bytes of the data from the first byte of the common until the end of the packet
     */
    size_t getDataLen() const { return m_DataLen; }

    /**
     * @return A pointer for the layer payload, meaning the first byte after the common
     */
    ...
    /**
     * @return The common length in bytes
     */
  ...
  protected:
    ...
    AbstractLayer &operator=(const AbstractLayer &other);

    void setNextLayer(AbstractLayer *nextLayer) { m_NextLayer = nextLayer; }

    void setPrevLayer(AbstractLayer *prevLayer) { m_PrevLayer = prevLayer; }
...
inline std::ostream &operator<<(std::ostream &os, const pcpp::AbstractLayer &layer) {
  os << layer.toString();
  return os;
}
