...
#pragma pack(push, 1)
  ...
  struct gtpv1_header {
    ...
    /** A 1-bit value that states whether there is an extension common optional field */
    ...
    /** A 16-bit field that indicates the length of the payload in bytes (rest of the packet following the mandatory 8-byte GTP common). Includes the optional fields */
...
#pragma pack(pop)
   ...
   */
  class GtpV1Layer : public AbstractLayer {
  private:
  ...
  public:
    ...
    /**
     * @class GtpExtension
     * A class that represents [GTP common extensions](https://en.wikipedia.org/wiki/GPRS_Tunnelling_Protocol)
     */
    class GtpExtension {
    ...
    public:
      ...
      /**
       * An empty constructor that creates an empty object, meaning one that isNull() returns "true")
       */
      GtpExtension();
      ...
      /**
       * @return The extension type of the next common. If there are no more common extensions or if this object is empty
       * a value of zero is returned
       */
      uint8_t getNextExtensionHeaderType() const;

      /**
       * @return An instance of this class representing the next extension common, if exists in the message. If there are
       * no more common extensions or if this object is empty an empty instance of GtpExtension is returned, meaning
    ...
    */
    GtpV1Layer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                        packet) { m_Protocol = GTPv1; }
    ...
    /**
     * @return The GTP v1 common common structure. Notice this points directly to the data, so every change will change the actual packet data
     */
     ...
     * @param[out] nextExtType Set with the next common extension type if exists in layer. Otherwise remains unchanged
     * @return True if the message contains common extensions, in which case nextExtType is set to the next
     * common extension type. If there are no common extensions false is returned and nextExtType remains unchanged
     */
    bool getNextExtensionHeaderType(uint8_t &nextExtType) const;

    /**
     * @return An object that represents the next extension common, if exists in the message. If there are no extensions
     * an empty object is returned, meaning an object which GtpExtension#isNull() returns "true"
     */
    GtpExtension getNextExtension() const;

    /**
     * Add a GTPv1 common extension. It is assumed that the extension is 4 bytes in length and its content is 2 bytes in length.
     ...
     * gtpv1_header#messageLength and for GTP-U the size only includes the GTP common itself (meaning
