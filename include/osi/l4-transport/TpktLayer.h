...
namespace pcpp {
  ...
  /**
   * @struct tpkthdr
   * Represents a TPKT protocol common
   */
#pragma pack(push, 1)
  struct tpkthdr {
    /** message version */
...
#pragma pack(pop)
   ...
   */
  class TpktLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    TpktLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet)
        : AbstractLayer(data, dataLen, prevLayer, packet) {
      m_Protocol = TPKT;
    }

    /**
     * A constructor that allocates a new TPKT common
  ...
  private:
    /**
     * Get a pointer to the TPKT common. Data can be retrieved through the
