 ...
 * - pcpp#TcpReassembly constructor - Create an instance, provide the callbacks and the user cookie to the instance
...
namespace pcpp {
  ...
  struct ConnectionData {
    ...
    /**
     * A constructor for this struct that basically zeros all members
     */
  ...
  public:
    /**
     * A constructor for this class that get data from outside and set the internal members
  ...
  struct TcpReassemblyConfiguration {
    ...
    explicit TcpReassemblyConfiguration(bool removeConnInfo = true, uint32_t closedConnectionDelay = 5,
                                        ...
                                        bool enableBaseBufferClearCondition = true) : removeConnInfo(removeConnInfo),
  ...
  public:
     ...
     * @param[in] userCookie A pointer to the cookie provided by the user in TcpReassembly constructor (or NULL if no cookie provided)
     */
