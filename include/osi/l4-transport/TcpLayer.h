...
namespace pcpp {
  ...
  /**
   * @struct tcphdr
   * Represents an TCP protocol common
   */
#pragma pack(push, 1)
  struct tcphdr {
    /** Source TCP port */
    ...
    uint16_t reserved: 4,
    /** Specifies the size of the TCP common in 32-bit words */
    dataOffset: 4,
    /** FIN flag */
    finFlag: 1,
    /** SYN flag */
...
#error	"Endian is not LE nor BE..."
    ...
    uint16_t windowSize;
    /** The 16-bit checksum field is used for error-checking of the common and data */
...
#define PCPP_TCPOLEN_EXP_MIN        2
   ...
   * A class for building TCP option records. This builder receives the TCP option parameters in its constructor,
  ...
  public:
    ...
    /**
     * A constructor for building TCP options which their value is a byte array. The TcpOption object can be later
    ...
    /**
     * A constructor for building TCP options which have a 1-byte value. The TcpOption object can be later retrieved
     * by calling build()
     * @param[in] optionType TCP option type
    ...
    /**
     * A constructor for building TCP options which have a 2-byte value. The TcpOption object can be later retrieved
     * by calling build()
     * @param[in] optionType TCP option type
    ...
    /**
     * A constructor for building TCP options which have a 4-byte value. The TcpOption object can be later retrieved
     * by calling build()
     * @param[in] optionType TCP option type
    ...
    /**
     * A constructor for building TCP NOP and EOL options. These option types are special in that they contain only 1 byte
    ...
    /**
     * Build the TcpOption object out of the parameters defined in the constructor
   ...
   */
  class TcpLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    TcpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet);

    /**
     * A constructor that allocates a new TCP common with zero TCP options
     */
    TcpLayer();

    /**
     * A constructor that allocates a new TCP common with source port and destination port and zero TCP options
    ...
    /**
     * An assignment operator that first delete all data from current layer and then copy the entire common from the other TcpLayer (including TCP options)
     */
    TcpLayer &operator=(const TcpLayer &other);

    /**
     * Get a pointer to the TCP common. Notice this points directly to the data, so every change will change the actual packet data
    ...
    /**
     * Calculate the checksum from common and data and possibly write the result to @ref tcphdr#headerChecksum
