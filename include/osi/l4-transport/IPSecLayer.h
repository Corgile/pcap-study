...
#pragma pack(pop)
   ...
   */
  class AuthenticationHeaderLayer : public AbstractLayer {
  public:
    ...
    */
    AuthenticationHeaderLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen,
                                                                                                                       prevLayer,
                                                                                                                       packet) { m_Protocol = AuthenticationHeader; }

    /**
     * Get a pointer to the raw AH common. Notice this points directly to the data, so every change will change the actual packet data
   ...
   */
  class ESPLayer : public AbstractLayer {
  public:
    /** A constructor that creates the layer from an existing packet raw data
    * @param[in] data A pointer to the raw data
    * @param[in] dataLen Size of the data in bytes
    ...
    * @param[in] packet A pointer to the Packet instance where layer will be stored in
    */
    ESPLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = ESP; }
