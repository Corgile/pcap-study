...
namespace pcpp {
  ...
  /**
   * @struct udphdr
   * Represents an UDP protocol common
   */
#pragma pack(push, 1)
  struct udphdr {
    /** Source port */
    ...
    /** Destination port */
    uint16_t portDst;
    /** Length of common and payload in bytes */
    uint16_t length;
    /**  Error-checking of the common and data */
...
#pragma pack(pop)
   ...
   */
  class UdpLayer : public AbstractLayer {
  public:
    /**
     ...
     */
    UdpLayer(uint8_t *data, size_t dataLen, AbstractLayer *prevLayer, Packet *packet) : AbstractLayer(data, dataLen, prevLayer,
                                                                                                      packet) { m_Protocol = UDP; }

    /**
     * A constructor that allocates a new UDP common with source and destination ports
    ...
    /**
     * Get a pointer to the UDP common. Notice this points directly to the data, so every change will change the actual packet data
    ...
    /**
     * Calculate the checksum from common and data and possibly write the result to @ref udphdr#headerChecksum
