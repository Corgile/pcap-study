...
namespace pcpp {
  ...
  public:
     ...
     */
    PcapRemoteAuthentication(const std::string &username, const std::string &password) : userName(
        username) { this->password = password; }

    /**
     * A copy constructor for this object
     * @param[in] other The object to copy from
     */
    PcapRemoteAuthentication(const PcapRemoteAuthentication &other) : userName(other.userName),
  ...
  private:
    ...
    // constructor is private, as only PcapRemoteDeviceList should create instances of it, and it'll create only one for every remote interface
    PcapRemoteDevice(pcap_if_t *iface, PcapRemoteAuthentication *remoteAuthentication, const IPAddress &remoteMachineIP,
