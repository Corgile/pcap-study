...
#define PCPP_RSS_HASH_MAGIC_NUMBER 0x123456
   ...
   * statistics, etc. This class has no public constructor as it's constructed by DpdkDeviceList during initialization.<BR>
   *
  ...
  public:
    ...
    struct DpdkDeviceConfiguration {
       ...
       * This parameter points to an array holding the RSS key to use for hashing specific common fields of received packets.
       ...
       * @param[in] rssKey A pointer to an array holding the RSS key to use for hashing specific common of received packets. If not
     ...
     */
    uint16_t
    sendPackets(MBufRawPacket **rawPacketsArr, uint16_t arrLength, uint16_t txQueueId = 0, bool useTxBuffer = false);
  ...
  private:
    ...
    uint16_t
    sendPacketsInner(uint16_t txQueueId, void *packetStorage, PacketIterator iter, int arrLength, bool useTxBuffer);
