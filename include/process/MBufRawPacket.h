...
#define MBUFRAWPACKET_OBJECT_TYPE 1
  ...
  public:
    ...
    /**
     * A default constructor for this class. Constructs an instance of this class without an mbuf attached to it. In order to allocate
     * an mbuf the user should call the init() method. Without calling init() the instance of this class is not usable.
     * This constructor can be used for initializing an array of MBufRawPacket (which requires an empty constructor)
     */
    MBufRawPacket()
        : RawPacket(), m_MBuf(NULL), m_Mempool(NULL), m_FreeMbuf(true) { m_DeleteRawDataAtDestructor = false; }
    ...
    /**
     * A copy constructor for this class. The copy constructor allocates a new mbuf from the same pool the original mbuf was
