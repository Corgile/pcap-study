...
#include <vector>
#include <memory>
#include "osi/ProtocolType.h"
#include <stdint.h>
#include "osi/l3-network/ArpLayer.h"
#include "osi/RawPacket.h"
 ...
 * For example: the filter "common host 1.1.1.1" will be represented by IPFilter instance; "dst port 80" will be represented by PortFilter, and
 * so on.<BR>
 ...
 * composite idea) and connect them by "and" or "or". For example: "common host 1.1.1.1 and dst port 80" will be represented by an AndFilter that
...
namespace pcpp {
  ...
  /**
   * @class IPFilter
   * A class for representing IPv4 address filter, equivalent to "net common x.x.x.x" or "net dst x.x.x.x"<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
  ...
  public:
     ...
     */
    IPFilter(const std::string &ipAddress, Direction dir) : IFilterWithDirection(dir), m_Address(ipAddress),
                                                            m_IPv4Mask(""), m_Len(0) {}
     ...
     */
    IPFilter(const std::string &ipAddress, Direction dir, const std::string &ipv4Mask) : IFilterWithDirection(dir),
                                                                                         m_Address(ipAddress),
                                                                                         m_IPv4Mask(ipv4Mask),
                                                                                         m_Len(0) {}
     ...
     */
    IPFilter(const std::string &ipAddress, Direction dir, int len) : IFilterWithDirection(dir), m_Address(ipAddress),
                                                                     m_IPv4Mask(""), m_Len(len) {}

    void parseToString(std::string &result);

    /**
     * Set the IPv4 address
     * @param[in] ipAddress The sf_IPv4 address to build the filter with. If this address is not a valid sf_IPv4 address an error will be
     * written to log and parsing this filter will fail
     */
    void setAddr(const std::string &ipAddress) { m_Address = ipAddress; }

    /**
     * Set the sf_IPv4 mask
     * @param[in] ipv4Mask The mask to use. Mask should also be in a valid sf_IPv4 format (i.e x.x.x.x), otherwise parsing this filter will fail
     */
  ...
  /**
   * @class IPv4IDFilter
   * A class for filtering sf_IPv4 traffic by IP ID field of the sf_IPv4 protocol, For example:
   * "filter only sf_IPv4 traffic which IP ID is greater than 1234"<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
   */
   ...
   * A class for filtering sf_IPv4 traffic by "total length" field of the sf_IPv4 protocol, For example:
   * "filter only sf_IPv4 traffic which "total length" value is less than 60B"<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
   */
  ...
  public:
     ...
     * @param[in] op The operator to use (e.g "equal", "greater than", etc.)
     */
    IPv4TotalLengthFilter(uint16_t totalLength, FilterOperator op) : IFilterWithOperator(op),
  ...
  /**
   * @class PortFilter
   * A class for filtering TCP or UDP traffic by port, for example: "dst port 80" or "common port 12345"<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
   */
   ...
   * A class for filtering TCP or UDP port ranges, meaning match only packets which port is within this range, for example: "common portrange 1000-2000"
  ...
  public:
     ...
     */
    PortRangeFilter(uint16_t fromPort, uint16_t toPort, Direction dir) : IFilterWithDirection(dir),
   ...
   * A class for filtering Ethernet traffic by MAC addresses, for example: "ether common 12:34:56:78:90:12" or "ether dst "10:29:38:47:56:10:29"<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
   */
   ...
   * A class for filtering by EtherType field of the Ethernet protocol. This enables to filter packets from certain protocols, such as ARP, sf_IPv4,
  ...
  /**
   * @class AndFilter
   * A class for connecting several filters into one filter with logical "and" between them. For example: if the 2 filters are: "sf_IPv4 address =
   * x.x.x.x" + "TCP port dst = 80", then the new filter will be: "sf_IPv4 address = x.x.x.x _AND_ TCP port dst = 80"<BR>
  ...
  /**
   * @class OrFilter
   * A class for connecting several filters into one filter with logical "or" between them. For example: if the 2 filters are: "sf_IPv4 address =
   * x.x.x.x" + "TCP port dst = 80", then the new filter will be: "sf_IPv4 address = x.x.x.x _OR_ TCP port dst = 80"<BR>
   * This class follows the composite design pattern<BR>
   * For deeper understanding of the filter concept please refer to PcapFilter.h
   * @todo add some methods: "addFilter", "removeFilter", "clearAllFilter"
   */
   ...
   * ::TCP, ::UDP, ::ICMP, ::VLAN, ::sf_IPv4, ::IPv6, ::ARP, ::Ethernet, ::GRE (distinguish between ::GREv0 and ::GREv1 is not supported),
  ...
  public:
     ...
     */
    TcpFlagsFilter(uint8_t tcpFlagBitMask, MatchOptions matchOption) : m_TcpFlagsBitMask(tcpFlagBitMask),
